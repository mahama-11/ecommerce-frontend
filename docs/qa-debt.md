...
|---|---|---|---|---|---|
| QA-DEBT-001 | P1 | 真实契约 | ecommerce backend real API | 已升级默认 `npm run ci:qa-gate`：真实 contract 缺 authenticated fixture 会 fail-closed，且必须跑 `npm run qa:business:live` 写入/回读/cleanup。 | Dev/review lane 需要提供 auth fixture；默认 live provider 为 `comfyui_bridge`，可用 `ECOMMERCE_LIVE_QA_PROVIDER` 覆盖。 |
