...
send_files() {
  ...
  #scp -i "$SSH_KEY" -r deploy/ecommerce-nginx.dev.conf "$REMOTE:$REMOTE_DIR/ecommerce-nginx.dev.conf"
...
prune_local_tars() {
  ...
  count=$(ls -1t "$dir"/*ecommerce*.tar.gz 2>/dev/null | wc -l | tr -d ' ')
...
prune_remote_prod_images() {
  ...
  remote "cur=\$(docker inspect -f '{{.Config.Image}}' v-ecommerce-frontend 2>/dev/null || echo ''); \
  rep='${IMAGE_NAME}'; \
...
dev)
  ...
  health_wait "v-ecommerce-frontend-dev" 10
...
prod)
  ...
  remote "cur=\$(docker inspect -f '{{.Config.Image}}' v-ecommerce-frontend 2>/dev/null || echo ''); if [ -n \"\$cur\" ]; then tag=\$(echo \"\$cur\" | awk -F: '{print \$2}'); [ -n \"\$tag\" ] && docker save \"\$cur\" | gzip > $REMOTE_BASE/backups/${IMAGE_NAME##*/}_\$tag.tar.gz && echo \"\$tag\" > $REMOTE_DIR/.prod_prev; fi || true"
  ...
  remote "cd $REMOTE_DIR; PROD_TAG=$PROD_TAG_IN docker compose up -d prod-ecommerce-web"
...
promote)
  ...
  remote "docker rm -f v-ecommerce-frontend 2>/dev/null || true"
  remote "cd $REMOTE_DIR; PROD_TAG=$NEW_TAG docker compose up -d prod-ecommerce-web"
  health_wait "v-ecommerce-frontend" 60
  prune_remote_prod_images 3
  ;;
rollback)
  remote "cd $REMOTE_DIR; prev=\$(cat .prod_prev 2>/dev/null || echo 1.0.0); PROD_TAG=\$prev docker compose up -d prod-ecommerce-web"
  health_wait "v-ecommerce-frontend" 60
  ;;
*)
