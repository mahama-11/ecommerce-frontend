...
function usePolling<T>( fetcher: () => Promise<T>, shouldPoll: (data: T) => boolean, intervalMs = 2000,
  ...
  useEffect(() => { fetcherRef.current = fetcher
    shouldPollRef.current = shouldPoll }, [fetcher, shouldPoll])
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (shouldPollRef.current(result)) {
        ...
        timerRef.current = setTimeout(() => void start(), intervalMs) } else { startedAtRef.current = null }
    } catch (e) { setError(e instanceof Error ? e.message : 'Polling error') } finally { inFlightRef.current = false
      setLoading(false) } }, [intervalMs, maxDurationMs])
...
const ATTRIBUTE_LABEL_ZH: Record<string, string> = { product_info: '图片中的产品信息', background_info: '图片中的背景信息', provider_reference_description: '参考素材解析结果',
  reference_style: '参考素材风格', product_geometry: '商品形态', geometry: '商品形态', material: '材质',
  style: '风格', scene: '场景', brand_constraints: '品牌约束', lighting: '光线',
  composition: '构图', color: '色彩', props: '道具元素', background: '背景',
  reference_composition: '参考图构图', scene_reference: '参考图场景', analysis_limitation: '识别提醒', unverified_visual_claim: '待确认视觉点',
}
...
function localizeAttributeValue(value: unknown, language: string): string {
  ...
  const sanitized = text .replace(/Manifest-declared SKU visual asset; geometry analysis pending runtime image bytes/gi, '已识别为当前商品图片；系统会在生成时以实物图为准。') .replace(/Manifest-declared SKU asset; geometry not extractable without image bytes\.?/gi, '已识别为当前商品图片；系统会在生成时以实物图为准。') .replace(/Requested element:\s*product geometry analysis pending visual byte ingestion for asset_[A-Za-z0-9_-]+/gi, '当前图片可以作为商品主体，但外形细节还需要在生成前再次确认。')
    .replace(/Requested element:\s*material analysis pending visual byte ingestion for asset_[A-Za-z0-9_-]+/gi, '当前图片可以作为材质参考，但具体材质还需要确认。') .replace(/Reference asset available:\s*asset_[A-Za-z0-9_-]+\s*\([^)]*\)\s*for comparative visual analysis/gi, '参考图已就绪，可用于对比风格、场景和构图。') .replace(/All visual facts unverified:.*$/gi, '当前还有部分视觉细节无法自动确认，请以你上传的商品图为准。') .replace(/visual byte ingestion/gi, '图片细节识别')
    .replace(/comparative visual analysis/gi, '视觉对比') .replace(/bounding boxes requested but cannot be generated without pixel data/gi, '细节位置还需要图片内容确认') .replace(/image bytes/gi, '图片内容') .replace(/runtime/gi, '生成服务')
    .replace(/backend/gi, '系统') .replace(/Manifest/gi, '图片信息') .replace(/SKU asset/gi, '商品图片') .replace(/contract[-_ ]needed/gi, '暂不可用')
    .replace(/prompt[_ ]plan/gi, '生成方案')
  if (!language.toLowerCase().startsWith('zh')) return sanitized
  return sanitized .replace(/industrial workshop/gi, '工业车间') .replace(/warm backlight/gi, '暖色逆光') .replace(/low angle/gi, '低角度')
    .replace(/metallic/gi, '金属质感') .replace(/minimal/gi, '极简') .replace(/white background/gi, '白色背景') .replace(/Manifest-declared SKU asset; geometry not extractable without image bytes\.?/gi, '已识别为当前商品图片；系统会在生成时以实物图为准。')
    .replace(/geometry not extractable without image bytes\.?/gi, '图片细节不足，建议确认商品外形。') .replace(/backend/gi, '系统') .replace(/runtime/gi, '生成服务') .replace(/contract[-_ ]needed/gi, '暂不可用')
    .replace(/prompt[_ ]plan/gi, '生成方案') .replace(/Manifest/gi, '图片信息') .replace(/SKU asset/gi, '商品图片') }
// ─── Upload Zone Component ──────────────────────────────────
function UploadZone({ title, icon: Icon, iconColor,
  hint, sources, uploading, onDrop,
  onFileInput, onRemove, inputRef, testId,
}: { title: string
  icon: typeof Upload
  iconColor: string
  hint: string
  ...
  testId?: string }) {
  return ( <div data-testid={testId} className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
      ...
      >
        {sources.length > 0 ? ( <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            ...
            <div className="flex aspect-square flex-col items-center justify-center rounded-lg border border-dashed border-white/[0.08] bg-white/[0.01] text-white/25">
              ...
              <span className="text-[9px]">继续添加/替换</span> </div> </div> ) : uploading ? (
...
function AttributeRow({ attr, decision }: { attr: ParsedAttribute; decision?: AttentionDecision }) {
  ...
  const decisionLabel = decision === 'keep' ? '已纳入出图要求' : decision === 'replace' ? '以当前 SKU 替换主体'
      : decision === 'crop' ? '局部/裁剪纳入出图要求' : decision === 'drop' ? '已排除，不进入出图要求'
        : '待确认取舍'
  const decisionClass = decision === 'keep' ? 'border-violet-300/15 bg-violet-400/10 text-violet-100/75' : decision === 'replace' ? 'border-cyan-300/15 bg-cyan-400/10 text-cyan-100/75'
      : decision === 'drop' ? 'border-slate-300/10 bg-white/[0.035] text-white/35' : 'border-amber-300/15 bg-amber-400/10 text-amber-100/70'
  return ( <div className="rounded-lg border border-white/[0.04] bg-white/[0.015] px-3 py-2">
      <div className="mb-1 flex min-w-0 items-center gap-2">
        <span className="min-w-0 flex-1 truncate text-[11px] font-medium text-white/70" title={label}>
          {label} </span>
        <span
          className={`shrink-0 rounded px-1 py-0.5 text-[9px] font-medium ${ attr.source === 'comfyui' ? 'bg-cyan-400/10 text-cyan-400' : 'bg-amber-400/10 text-amber-400'
          }`}
        >
          {roleLabel} </span> </div>
...
export default function PrepHubPage() {
  ...
  const { productId, sources, parsing,
    decisionTree, globalDriftBias, setProductId, addSource,
    setSources, setParsing, setDecisionTree, setGlobalDriftBias,
    reset, } = usePrepStore()
  ...
  useEffect(() => {
    ...
    let cancelled = false
    productionApi.listParsingSources(productId) .then((items) => {
        if (cancelled) return
        items.forEach((source) => addSource(source)) }) .catch(() => {})
    return () => { cancelled = true } }, [productId, sources.length, addSource])
  ...
  useEffect(() => {
    ...
    let cancelled = false
    Promise.all([ productionApi.getParsingResult(productId), productionApi.getDecisionTree(productId), ])
      .then(([result, tree]) => {
        if (cancelled || result.status === 'idle') return
        setParsing(result)
        if (result.status === 'succeeded' && tree.status !== 'idle') setDecisionTree(tree) }) .catch(() => {
        if (hydratedProductRef.current === productId) hydratedProductRef.current = null })
    return () => { cancelled = true } }, [productId, parsing, decisionTree, setParsing, setDecisionTree])
  // ─── File upload ────────────────────────────────────────
  ...
  const handleFiles = useCallback( async (files: FileList | File[], sourceType: 'sku_image' | 'reference_image') => {
      if (!productId) return
      setUploading(true)
      try {
        for (const file of Array.from(files)) {
          try { productionApi.validateParsingSourceFile(file) } catch (validationError) { toast.showToast(validationError instanceof Error ? validationError.message : '图片格式或大小不符合要求', 'error')
            continue }
          const source = await productionApi.uploadParsingSource(productId, file, sourceType)
          addSource({ ...source, type: sourceType }) } } catch (e) { toast.showToast(e instanceof Error ? e.message : 'Upload failed', 'error')
      } finally { setUploading(false) } },
    [productId, addSource, toast], )
  const onDropSku = useCallback( (e: React.DragEvent) => { e.preventDefault()
      void handleFiles(e.dataTransfer.files, 'sku_image') }, [handleFiles], )
  const onDropRef = useCallback( (e: React.DragEvent) => { e.preventDefault()
      void handleFiles(e.dataTransfer.files, 'reference_image') }, [handleFiles], )
  const onFileInputSku = useCallback( (e: React.ChangeEvent<HTMLInputElement>) => {
      if (e.target.files) void handleFiles(e.target.files, 'sku_image')
      e.target.value = '' }, [handleFiles], )
  const onFileInputRef = useCallback( (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleRemoveSource = useCallback(async (sourceId: string) => {
    ...
    try {
      ...
      if (next.length === 0) { setParsing(null)
        setDecisionTree(null) } } catch (e) { setSources(previous)
      toast.showToast(e instanceof Error ? e.message : '删除图片失败，请稍后重试。', 'error') } }, [productId, sources, setSources, setParsing, setDecisionTree, toast])
  // ─── Parsing ────────────────────────────────────────────
  const parsingPoll = usePolling<DualTrackParsing>( () => productionApi.getParsingResult(productId!), (d) => d.status === 'parsing', )
  ...
  useEffect(() => {
    ...
    parsingPoll.start() }, [productId, parsing?.status, parsingPoll.error, parsingPoll.start])
  // ─── Dev mode: auto-fill mock sources & trigger parsing ──
  const devAutoFilled = useRef(false)
  useEffect(() => {
    if ( import.meta.env.DEV && window.location.search.includes('dev=1') && productId &&
      sources.length === 0 && !parsing && !devAutoFilled.current ) {
      devAutoFilled.current = true
      MOCK_SOURCES.forEach((s) => addSource(s))
      setTimeout(async () => {
        try {
          await productionApi.startParsing({ productId, sourceIds: MOCK_SOURCES.map((s) => s.id), tracks: ['comfyui', 'third_party'],
            providerCode: understandingProvider, })
          parsingPoll.start() } catch { /* ignore */ } }, 300) }
  }, [productId, sources.length, parsing, addSource, parsingPoll, understandingProvider])
  const startParsing = useCallback(async () => {
    if (!productId) return
    if (skuSources.length === 0 || refSources.length === 0) { toast.showToast('请先上传一张商品图和一张参考图，再开始解析。', 'error')
      return
    }
    try { setParsing({ status: 'parsing', primaryTrack: 'third_party',
        mergedAttributes: [], conflicts: [], thirdPartyResult: { track: 'third_party', status: 'parsing', attributes: [] }, })
      await productionApi.startParsing({ productId, sourceIds: sources.map((s) => s.id), tracks: ['comfyui', 'third_party'],
        providerCode: understandingProvider, })
      parsingPoll.start() } catch (e) {
      const message = e instanceof Error ? e.message : '解析失败，请稍后重试。'
      setParsing({ status: 'failed', primaryTrack: 'third_party', mergedAttributes: [],
        conflicts: [], thirdPartyResult: { track: 'third_party', status: 'failed', attributes: [], error: message }, })
      toast.showToast(message, 'error') } }, [productId, sources, skuSources.length, refSources.length, parsingPoll, setParsing, toast, understandingProvider])
  // Sync poll results into store
  useEffect(() => {
    if (parsingPoll.data) setParsing(parsingPoll.data) }, [parsingPoll.data, setParsing])
  // ─── Decision tree ──────────────────────────────────────
  const treePoll = usePolling<LlmDecisionTreeResult>( () => productionApi.getDecisionTree(productId!), (d) => d.status === 'evaluating', )
  ...
  useEffect(() => {
    ...
    let cancelled = false
    productionApi.getDecisionTree(productId) .then((tree) => {
        if (!cancelled && tree.status !== 'idle') setDecisionTree(tree) }) .catch(() => {
        if (decisionHydratingProductRef.current === productId) decisionHydratingProductRef.current = null })
    return () => { cancelled = true } }, [productId, parsing?.status, decisionTree, setDecisionTree])
  useEffect(() => {
    if (treePoll.data) setDecisionTree(treePoll.data) }, [treePoll.data, setDecisionTree])
  ...
  useEffect(() => { setSessionSelections({})
    setUsePreviousSelections(true)
    setCurrentStepIndex(0) }, [productId])
  const decisionProgress = useMemo(() => {
    ...
    return { total, answered: effectiveAnswered, historicalAnswered,
      remaining: Math.max(total - effectiveAnswered, 0), complete: total >= 4 && effectiveAnswered >= total, percent: total > 0 ? Math.round((effectiveAnswered / total) * 100) : 0, }
  ...
  const displaySteps = useMemo(() => {
    ...
    return rawSteps.map((step, idx) => {
      ...
      return { ...step, selectedOptionId, status: isAnswered ? 'completed' as const : idx === activeIndex ? 'active' as const : 'pending' as const,
      } }) }, [decisionTree?.steps, sessionSelections, currentStepIndex])
  useEffect(() => {
    if (displaySteps.length === 0) return
    if (currentStepIndex >= displaySteps.length) { setCurrentStepIndex(displaySteps.length - 1)
      return
    }
    ...
    if (current?.selectedOptionId && nextUnanswered >= 0) setCurrentStepIndex(nextUnanswered) }, [displaySteps, currentStepIndex])
  const handleSelectOption = useCallback( async (stepId: string, optionId: string) => {
      ...
      if (decision === 'replace' && !targetAssetId) { toast.showToast('选择“换成我的商品”前，请先上传商品图。', 'error')
        return
      }
      ...
      try {
        ...
        const [updatedParsing, updatedTree] = await Promise.all([ productionApi.getParsingResult(productId), productionApi.getDecisionTree(productId), ])
        setParsing(updatedParsing)
        setDecisionTree(updatedTree) } catch (e) { setUsePreviousSelections(previousUsePreviousSelections)
  ...
  // ─── Navigate to Sandbox ────────────────────────────────
  const goToSandbox = () => {
    if (!prepCanEnterSandbox) { toast.showToast(prepNextStepHint, 'info')
      return
    }
  ...
  const commitDriftBias = useCallback(async (value: number = globalDriftBias) => {
    if (!productId) return
    try {
      await productionApi.updateDriftControl(productId, value) } catch (e) { toast.showToast(e instanceof Error ? e.message : 'Failed to persist attribute drift control', 'error') }
  }, [productId, globalDriftBias, toast])
  const handleDriftBiasChange = useCallback((value: number) => { setGlobalDriftBias(value) }, [setGlobalDriftBias])
  ...
  const dualTrackBlocker = !hasSkuSource ? '请先上传一张商品图。' : !hasReferenceSource ? '请再上传一张参考图。'
      : '可以开始解析。'
  ...
  const parsingQualityWarning = hasLowConfidenceAttributes && !decisionProgress.complete ? `图片识别已返回，但有 ${lowConfidenceAttributes.length} 项可信度较低，请核对四问说明后再继续。` : null
  const parsingQualityReviewedMessage = hasLowConfidenceAttributes && decisionProgress.complete ? `已通过四问核对 ${lowConfidenceAttributes.length} 项低可信识别结果，下一步会按你的选择合成本次出图要求。` : null
  const prepCanEnterSandbox = parsing?.status === 'succeeded' && decisionProgress.complete
  const prepNextStepHint = parsing?.status !== 'succeeded' ? '请先完成商品图和参考图解析。' : !decisionProgress.complete ? (parsingQualityWarning ?? `还差 ${decisionProgress.remaining || 4} 个“要/不要”选择，请先完成出图四问。`)
      : '可以进入策略配置。'
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-12">
          ...
          <div className="rounded-2xl border border-cyan-300/20 bg-cyan-400/[0.055] p-4 shadow-[0_18px_55px_rgba(34,211,238,0.10)]">
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[11px] leading-relaxed text-cyan-100/55">
                  上传商品图和参考图后，系统会识别可用于出图的商品事实，并生成后续的视觉策略取舍。 </p> </div>
              {dualTrackReady ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-cyan-200/70" /> : <AlertCircle className="mt-0.5 h-4 w-4 shrink-0 text-amber-200/70" />} </div>
            <div className="mb-3 rounded-xl border border-white/[0.06] bg-black/20 p-3">
              <div className="mb-1 flex items-center justify-between gap-2">
                ...
                <span className="rounded bg-white/[0.05] px-1.5 py-0.5 text-[9px] text-white/35">可切换识别侧重</span> </div>
              <select
          ...
          {isParsing && ( <div className="relative overflow-hidden rounded-xl border border-cyan-300/25 bg-cyan-400/[0.08] px-4 py-3 shadow-[0_0_28px_rgba(34,211,238,0.12)]">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-[10px] text-cyan-100/55">图片正在识别中，完成后会自动更新中间选择和右侧属性。</p> </div> </div> </div>
          )}
          {parsingBlocked && parsingBlockerMessage && ( <div className="rounded-xl border border-rose-300/25 bg-rose-400/[0.08] px-4 py-3 text-xs leading-relaxed text-rose-100">
              <div className="mb-1 flex items-center gap-2 font-semibold">
                <AlertCircle className="h-4 w-4" />
                图片识别未完成 </div>
              <p className="text-rose-100/75">{parsingBlockerMessage}</p> </div> )}
          {parsingQualityWarning && ( <div className="rounded-xl border border-amber-300/25 bg-amber-400/[0.08] px-4 py-3 text-xs leading-relaxed text-amber-100">
              <div className="mb-1 flex items-center gap-2 font-semibold">
                <AlertCircle className="h-4 w-4" />
                识别结果需要核对 </div>
              <p className="text-amber-100/75">{parsingQualityWarning}</p> </div> )}
          {parsingQualityReviewedMessage && ( <div className="rounded-xl border border-emerald-300/20 bg-emerald-400/[0.07] px-4 py-3 text-xs leading-relaxed text-emerald-100">
              <div className="mb-1 flex items-center gap-2 font-semibold">
                <CheckCircle2 className="h-4 w-4" />
                低可信项已核对 </div>
              <p className="text-emerald-100/75">{parsingQualityReviewedMessage}</p> </div> )}
          {/* Conflict indicator */}
          {parsing?.conflicts && parsing.conflicts.length > 0 && ( <div className="rounded-lg border border-amber-400/15 bg-amber-400/[0.04] px-3 py-2.5">
              <div className="mb-1 flex items-center gap-1.5">
                <AlertCircle className="h-3.5 w-3.5 text-amber-400" />
                <span className="text-[11px] font-medium text-amber-400">
                  {parsing.conflicts.length} conflict{parsing.conflicts.length > 1 ? 's' : ''} </span> </div>
              {parsing.conflicts.map((c, i) => ( <div key={i} className="text-[10px] text-white/40">
                  <span className="font-medium text-white/50">{c.key}</span>:{' '}
                  {String(c.comfyuiValue)} vs {String(c.thirdPartyValue)}
                  {c.resolvedValue != null && ( <span className="text-emerald-400/60">
                      {' → '}{String(c.resolvedValue)} </span> )} </div>
              ))} </div> )} </motion.div>
        ...
        >
          <div className="mb-4 flex items-center justify-between">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-white">
              ...
              {t('production.prep.decisionTree')} </h2> </div>
          {isEvaluating ? ( <div className="flex min-h-[300px] flex-col items-center justify-center">
              <Loader2 className="mb-2 h-8 w-8 animate-spin text-violet-400/40" />
              <p className="text-xs text-white/40">
                {t('production.prep.evaluating')} </p> </div> ) : steps.length > 0 ? (
            <div className="space-y-4">
              ...
              <div className="rounded-xl border border-violet-400/10 bg-violet-400/[0.04] p-3">
                ...
                {decisionProgress.complete && ( <div className="mt-3 rounded-lg border border-emerald-300/15 bg-emerald-300/[0.05] p-2.5 text-[10px] leading-relaxed text-emerald-100/70">
                    策略取舍已确认完成。需要调整时可点“上一项”回看修改；也可以继续进入策略配置。 </div> )} </div>
              {/* Current step detail */}
              <AnimatePresence mode="wait">
                ...
                >
                  {steps[currentStepIndex] && ( <DecisionStepCard
              ...
              {decisionTree?.overallConfidence != null && ( <div className="flex items-center gap-3 rounded-lg bg-white/[0.02] px-3 py-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex flex-wrap gap-1">
                      {steps .filter((s) => s.selectedOptionId) .map((s) => {
                  ...
                  <span className="shrink-0 text-sm font-bold tabular-nums text-violet-400">
                    {Math.round(decisionTree.overallConfidence * 100)}% </span> </div> )}
            </div> ) : parsingBlocked ? ( <div className="flex min-h-[300px] flex-col items-center justify-center rounded-xl border border-amber-400/15 bg-amber-400/[0.04] px-5 text-center">
              ...
              <p className="mt-2 max-w-sm text-[11px] leading-relaxed text-white/45">
                {parsingBlockerMessage || '图片已上传成功，但识别服务暂时不可用。系统不会用假结果继续下一步。'} </p> </div> ) : (
            <div className="flex min-h-[300px] flex-col items-center justify-center text-center">
              <AlertCircle className="mb-2 h-8 w-8 text-white/15" />
              <p className="text-xs text-white/30">
                {hasSources ? '点击“开始解析”，系统会识别商品形态、材质、风格和场景。' : t('production.prep.uploadFirst')} </p> </div> )}
        </motion.section>
        ...
        >
          <h2 className="mb-4 flex items-center gap-2 text-sm font-semibold text-white">
            ...
            {parsing?.mergedAttributes.length ? ( <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] font-normal text-white/30">
                {parsing.mergedAttributes.length} 项 </span> ) : null} </h2>
          {parsing?.mergedAttributes.length ? ( <div className="space-y-2">
              <div className="rounded-xl border border-emerald-400/10 bg-emerald-400/[0.035] p-3 text-[10px] leading-relaxed text-white/40">
                这里展示图片解析得到的 SKU 与参考素材信息。中间四问会把“要/不要”转成自然语言，并同步影响下一步出图要求。 </div>
              ...
              {parsing.mergedAttributes.map((attr) => {
                ...
                return <AttributeRow key={attr.key} attr={attr} decision={decision} /> })} </div> </div>
          ) : parsingBlocked ? ( <p className="rounded-lg border border-amber-400/15 bg-amber-400/[0.04] px-3 py-4 text-center text-[11px] leading-relaxed text-amber-200/70">
              图片已上传，但识别服务暂时不可用，所以这里不会展示假属性。 </p> ) : ( <p className="py-8 text-center text-xs text-white/30">
              {t('production.prep.noAttributesYet')} </p> )} </motion.section>
      </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p className="mt-0.5 text-[10px] text-white/30">
                {t('production.prep.globalBiasHint')} </p> </div>
          ...
          <div className="relative h-2.5 rounded-full bg-gradient-to-r from-cyan-400/30 via-white/10 to-amber-400/30">
            ...
            <input
              ...
              onKeyUp={(e) => {
                if (e.key === 'ArrowLeft' || e.key === 'ArrowRight' || e.key === 'Home' || e.key === 'End') { void commitDriftBias(Number(e.currentTarget.value)) } }}
              className="absolute inset-0 h-full w-full cursor-pointer appearance-none opacity-0"
            /> </div>
          <div className="mt-2.5 flex items-center justify-between text-[10px] text-white/25">
            <span className="flex items-center gap-1.5">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-cyan-400" />
              {t('production.prep.preserveSkuDetails')} </span>
            <span className="flex items-center gap-1.5">
              {t('production.prep.mimicReference')}
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-amber-400" /> </span> </div> </div>
      </motion.section>
