...
import { getProduct,
  ...
  updateListingVersion, adoptListingVersion,
  deleteProduct, deleteProductAsset,
  updateProductAssetRelation, listDownloads
...
function SelectField({ label, value, onChange, options, inline = false }: { label?: string, value: string, onChange: (v: string) => void, options: {label: string, value: string}[], inline?: boolean }) {
  return ( <div className={inline ? "flex items-center gap-2" : "space-y-1.5"}>
...
export function ProductDetailPage() {
  ...
  const copy = { basicInfo: t('product.detail.dossier.basicInfo'),
    autoSave: t('product.detail.dossier.autoSave'), saving: t('product.detail.dossier.saving'),
    skuCode: t('product.detail.dossier.skuCode'), currency: t('product.detail.dossier.currency'),
    title: t('product.detail.dossier.title'), category: t('product.detail.dossier.category'),
    brand: t('product.detail.dossier.brand'), tags: t('product.detail.dossier.tags'),
    addTag: t('product.detail.dossier.addTag'), addTagPlaceholder: t('product.detail.dossier.addTagPlaceholder'),
    noTags: t('product.detail.dossier.noTags'), productStatus: t('product.detail.dossier.productStatus'),
    assetStatus: t('product.detail.dossier.assetStatus'), listingStatus: t('product.detail.dossier.listingStatus'),
    exportStatus: t('product.detail.dossier.exportStatus'), openAIWorkspace: t('product.detail.dossier.openAIWorkspace'),
    openBatchListing: t('product.detail.dossier.openBatchListing'), openDownloadCenter: t('product.detail.dossier.openDownloadCenter'),
  }
  ...
  const [productForm, setProductForm] = useState<ProductFormState>({ skuCode: '',
    title: '', categoryId: '',
    brandId: '', costCurrency: 'USD',
    tags: [], })
  ...
  const [profitForm, setProfitForm] = useState({ platform: 'amazon',
    site: 'US', costPrice: 0,
    listingPrice: 0, logisticsCost: 0,
    platformFee: 0, otherFee: 0,
  })
  const [showExportModal, setShowExportModal] = useState(false)
  const [exportForm, setExportForm] = useState({ platform: 'amazon',
    site: 'US', locale: 'en_US',
    format: 'csv', })
  ...
  const [listingForm, setListingForm] = useState<{ versionLabel: string
    title: string
    description: string
    bulletPoints: string[]
    keywords: string[]
    ...
    locale: 'en_US', })
  useEffect(() => {
    if (id) { void loadProductWorkspace(id)
    } }, [id])
  async function loadProductWorkspace(productId: string) { setLoading(true)
    try {
      ...
      setSelectedDownloadId(current => ( current === 'all' || scopedDownloads.some(item => item.id === current) ? current : 'all'
      ))
      setProductForm({ skuCode: result.product.skuCode,
        title: result.product.title, categoryId: result.product.categoryId || '',
        brandId: result.product.brandId || '', costCurrency: result.product.costCurrency || 'USD',
        tags: result.product.tags || [], })
    } catch (error) { console.error('Failed to load product:', error)
      showToast(t('product.detail.toast.loadFailed'), 'error') } finally {
      setLoading(false) }
  }
  async function handleDelete() {
    if (!id) return
    if (!confirm(t('product.detail.deleteConfirm'))) { return
    }
    setDeleting(true)
    try {
      await deleteProduct(id)
      navigate('/products') } catch (error) {
      console.error('Failed to delete product:', error) } finally {
      setDeleting(false) }
  }
  async function persistProductPatch( patch: Partial<{
      skuCode: string
      title: string
      categoryId: string
      brandId: string
    ...
    try {
      ...
      setProductForm({ skuCode: updated.skuCode,
        title: updated.title, categoryId: updated.categoryId || '',
        brandId: updated.brandId || '', costCurrency: updated.costCurrency || 'USD',
        tags: updated.tags || [], })
      setProductSaveLabel(t('product.detail.savedJustNow')) } catch (error) {
      console.error('Failed to update product:', error)
      setProductSaveLabel(t('product.detail.saveFailed'))
      showToast(t('product.detail.toast.saveFailed'), 'error') } finally {
  ...
  async function handleCalculateProfit() {
    if (!id) return
    try {
      await calculateProfit(id, { platform: profitForm.platform,
        site: profitForm.site, costPrice: profitForm.costPrice,
        listingPrice: profitForm.listingPrice, logisticsCost: profitForm.logisticsCost,
        platformFee: profitForm.platformFee, otherFee: profitForm.otherFee,
      })
      setShowProfitModal(false)
      setProfitForm({ platform: 'amazon', site: 'US', costPrice: 0, listingPrice: 0, logisticsCost: 0, platformFee: 0, otherFee: 0 })
      await loadProductWorkspace(id) } catch (error) {
      console.error('Failed to calculate profit:', error) }
  }
  async function handleCreateExport() {
    if (!id) return
    try {
      await createExportTask(id, { platform: exportForm.platform,
        site: exportForm.site, locale: exportForm.locale,
        format: exportForm.format, assetRelationIds: exportScopedRelationIds.length > 0 ? exportScopedRelationIds : undefined,
      })
  ...
  function openExportModalForAllAssets() { setExportScopedRelationIds([])
  ...
  async function handleCreateListing() {
    ...
    try {
      if (editingVersion) {
        await updateListingVersion(id, editingVersion.id, { versionLabel: listingForm.versionLabel,
          title: listingForm.title, description: listingForm.description,
          bulletPoints: listingForm.bulletPoints.filter(Boolean), keywords: listingForm.keywords,
          platform: listingForm.platform, site: listingForm.site,
          locale: listingForm.locale, })
      } else {
        await createListingVersion(id, { versionLabel: listingForm.versionLabel,
          title: listingForm.title, description: listingForm.description,
          bulletPoints: listingForm.bulletPoints.filter(Boolean), keywords: listingForm.keywords,
          platform: listingForm.platform, site: listingForm.site,
          locale: listingForm.locale, })
      ...
      setListingForm({ versionLabel: '',
        title: '', description: '',
        bulletPoints: ['', '', '', '', ''], keywords: [],
        newKeyword: '', platform: 'amazon',
        site: 'US', locale: 'en_US',
      })
      await loadProductWorkspace(id) } catch (error) {
      console.error('Failed to create listing:', error) } finally {
      setSavingListing(false) }
  }
  function openCreateListingModal() { setEditingVersion(null)
    setListingForm({ versionLabel: '',
      title: '', description: '',
      bulletPoints: ['', '', '', '', ''], keywords: [],
      newKeyword: '', platform: 'amazon',
  ...
  function openEditListingModal(version: ListingVersion) { setEditingVersion(version)
    setListingForm({ versionLabel: version.versionLabel,
      title: version.title, description: version.description || '',
      bulletPoints: [...version.bulletPoints, '', '', '', '', ''].slice(0, 5), keywords: [...version.keywords],
      newKeyword: '', platform: version.platform,
      site: version.site, locale: version.locale,
    })
    setShowListingModal(true) }
  async function handleAdoptListing(versionId: string) {
    ...
    try {
      ...
      await loadProductWorkspace(id) } catch (error) {
      console.error('Failed to adopt listing:', error) } finally {
      setAdoptingVersionId(null) }
  }
  function addKeyword() {
    if (listingForm.newKeyword && !listingForm.keywords.includes(listingForm.newKeyword)) { setListingForm(prev => ({ ...prev, keywords: [...prev.keywords, prev.newKeyword], newKeyword: '' }))
    } }
  async function handleMakePrimaryAsset(assetRelationId: string) {
    ...
    try {
      ...
      showToast(t('product.detail.toast.primaryUpdated'), 'success') } catch (error) {
      console.error('Failed to update primary asset:', error)
      showToast(t('product.detail.toast.primaryUpdateFailed'), 'error') } finally {
  ...
  async function handleAssetRoleChange(assetRelationId: string, assetRole: string) {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.roleUpdated'), 'success') } catch (error) {
      console.error('Failed to update asset role:', error)
      showToast(t('product.detail.toast.roleUpdateFailed'), 'error') } finally {
      setAssetMutatingRelationId(null) }
  }
  async function handleDeleteAssetRelation(assetRelationId: string) {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.assetRemoved'), 'success') } catch (error) {
      console.error('Failed to delete product asset relation:', error)
      showToast(t('product.detail.toast.assetRemoveFailed'), 'error') } finally {
      setAssetMutatingRelationId(null) }
  }
  async function handleBulkAssetRoleChange(assetRelationIds: string[], assetRole: string) {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.bulkRoleUpdated', { count: assetRelationIds.length }), 'success') } catch (error) {
      console.error('Failed to bulk update asset role:', error)
      showToast(t('product.detail.toast.bulkRoleFailed'), 'error') } finally {
  ...
  async function handleBulkDeleteAssetRelations(assetRelationIds: string[]) {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.bulkRemoved', { count: assetRelationIds.length }), 'success') } catch (error) {
      console.error('Failed to bulk delete asset relations:', error)
      showToast(t('product.detail.toast.bulkRemoveFailed'), 'error') } finally {
      setAssetBulkMutating(false) }
  }
  async function handleAssetSortOrderChange(assetRelationId: string, sortOrder: number) {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.orderUpdated'), 'success') } catch (error) {
      console.error('Failed to update asset order:', error)
      showToast(t('product.detail.toast.orderUpdateFailed'), 'error') } finally {
      setAssetMutatingRelationId(null) }
  }
  async function handleMoveAsset(assetRelationId: string, direction: 'up' | 'down') {
    ...
    try {
      ...
      await loadProductWorkspace(id)
      showToast(t('product.detail.toast.moved'), 'success') } catch (error) {
      console.error(`Failed to move asset ${direction}:`, error)
      showToast(t('product.detail.toast.moveFailed'), 'error') } finally {
      setAssetMutatingRelationId(null) }
  }
  function handleInspectExportAssets(downloadId: string) { setSelectedDownloadId(downloadId)
  }
  void copy
  void deleting
  void savingProduct
    ...
    return ( <div className="min-h-[calc(100vh-72px)] bg-[var(--ecom-bg)] text-white p-6 flex items-center justify-center">
        <div className="text-center text-white/40">
          ...
          <Link to="/products" className="text-cyan-200 hover:underline mt-2 inline-block">
            {t('product.detail.backToProducts')} </Link>
        </div> </div>
    ) }
  const { product } = data
  ...
  return ( <div className="relative flex min-h-[calc(100vh-52px)] w-full flex-col overflow-hidden bg-[var(--ecom-bg)] text-[var(--ecom-text-primary)] font-sans">
      ...
      <style>{` .custom-scrollbar::-webkit-scrollbar { width: 6px; height: 6px; }
        .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.1); border-radius: 10px; }
        ...
        <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <Link to="/products" className="mb-3 inline-flex items-center gap-1.5 text-[13px] font-medium text-white/40 transition-colors hover:text-white/90">
              ...
              {t('productWorkbench.nav.productList')} </Link>
        ...
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
          <div className="space-y-5">
            <section className="rounded-[28px] border border-white/[0.07] bg-[var(--ecom-surface)] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.36)]">
              ...
              <div className="divide-y divide-white/[0.06] text-sm">
                {[ ['SKU', product.skuCode],
                  ...
                  ['成本', `${product.costCurrency || 'USD'} ${product.costJson || '—'}`], ['品牌', product.brandId || '—'],
                  ['标签', product.tags?.join(', ') || '—'], ['状态', product.status],
                  ['素材 / Listing / Export', `${product.assetStatus} / ${product.listingStatus} / ${product.exportStatus}`], ].map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between gap-6 py-2.5">
                    <span className="text-white/38">{label}</span>
                    <span className="max-w-[70%] truncate text-right font-medium text-white/82">{value}</span> </div>
                ))} </div>
            </section>
            <section className="rounded-[28px] border border-cyan-300/18 bg-cyan-300/[0.04] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.32)]">
              ...
              {parsedInfo.length > 0 ? ( <div className="divide-y divide-white/[0.06] text-sm">
                  {parsedInfo.slice(0, 5).map(item => ( <div key={item.key} className="grid gap-3 py-2.5 md:grid-cols-[180px_1fr]">
                      <span className="text-white/38">{item.key}</span>
                      <span className="text-white/78">{item.value}</span> </div>
                  ))} </div>
          ...
          <div className="bg-[var(--ecom-surface)] rounded-2xl w-full max-w-md border border-white/10 shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <SelectField label={t('product.detail.profitModal.site')} value={profitForm.site} onChange={v => setProfitForm({...profitForm, site: v})} options={[{label: t('product.detail.sites.US'), value: 'US'}, {label: t('product.detail.sites.CA'), value: 'CA'}, {label: t('product.detail.sites.UK'), value: 'UK'}]} /> </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <InputField label={t('product.detail.profitModal.otherFee')} value={String(profitForm.otherFee)} onChange={v => setProfitForm({...profitForm, otherFee: parseFloat(v) || 0})} /> </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4 text-xs text-white/58">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <span className="text-right text-cyan-100">{profitForm.listingPrice > 0 ? (((profitForm.listingPrice - profitForm.costPrice - profitForm.logisticsCost - profitForm.platformFee - profitForm.otherFee) / profitForm.listingPrice) * 100).toFixed(1) : '0.0'}%</span> </div>
          ...
          <div className="bg-[var(--ecom-surface)] rounded-2xl w-full max-w-md border border-white/10 shadow-2xl overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div className="rounded-xl border border-white/5 bg-white/[0.03] px-4 py-3 text-sm text-white/70">
                {exportScopedRelationIds.length > 0 ? t('product.detail.exportModal.scopeSelected', { count: exportScopedRelationIds.length })
                  : t('product.detail.exportModal.scopeAll', { count: data.assets.length })} </div>
          ...
          <div className="bg-[var(--ecom-surface)] rounded-2xl w-full max-w-lg border border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
              <div className="grid grid-cols-3 gap-4">
                ...
                <SelectField label={t('product.detail.listingModal.locale')} value={listingForm.locale} onChange={v => setListingForm({...listingForm, locale: v})} options={[{label: t('product.detail.locales.en_US'), value: 'en_US'}, {label: t('product.detail.locales.en_CA'), value: 'en_CA'}, {label: t('product.detail.locales.en_GB'), value: 'en_GB'}]} /> </div>
              ...
              <div className="space-y-1.5">
                ...
                <div className="space-y-2">
                  {listingForm.bulletPoints.map((point, index) => ( <input
                      key={index}
                      type="text"
                      value={point}
                      onChange={(e) => {
...
function DetailMetric({ label, value }: { label: string; value: string }) {
  return ( <div className="rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3">
      <div className="text-[10px] uppercase tracking-wider text-white/40">{label}</div>
      <div className="mt-1.5 text-sm font-medium text-white/90">{value}</div> </div>
  ) }
