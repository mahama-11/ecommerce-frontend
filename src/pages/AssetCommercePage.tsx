...
const PAGE_CONFIG: Record<string, PageConfig> = {
  '/database/knowledge': {
    ...
    icon: Database,
    badge: { zh: 'AI 可消费知识层', en: 'AI-Consumable Knowledge Layer' },
    subtitle: {
      zh: '把品牌、平台、合规和 SOP 资料沉淀成知识资产，为问答和模板提供上下文。',
      en: 'Turn brand, platform, compliance, and SOP content into knowledge assets for chat and templates.',
    },
    stats: [
      { value: '248', label: { zh: '知识条目', en: 'Knowledge Entries' } },
      { value: '16', label: { zh: '主题分类', en: 'Topic Buckets' } },
      { value: '12', label: { zh: '最近更新', en: 'Recent Updates' } },
    ],
    collections: [
      {
        title: { zh: '品牌规则手册', en: 'Brand Rule Handbook' },
        desc: { zh: '管理语气、禁用词、卖点描述和营销边界。', en: 'Manage tone, restricted terms, selling-point phrasing, and marketing boundaries.' },
        meta: { zh: '适合知识库问答', en: 'Best for knowledge chat grounding' },
      },
      {
        title: { zh: '平台合规文档', en: 'Platform Compliance Docs' },
        desc: { zh: '沉淀站点政策、敏感词规则和发布约束。', en: 'Store marketplace policies, risk terms, and publishing constraints.' },
        meta: { zh: '后续接引用式回答', en: 'Ready for citation answers' },
      },
    ],
    actions: [
  ...
  '/database/picturelibrary': {
    titleKey: 'pages.databasePicturelibrary',
    icon: ImageIcon,
    badge: { zh: '图片资产中心', en: 'Image Asset Center' },
    subtitle: {
      zh: '管理白底图、场景图、模特图、参考图和视频封面等视觉素材。',
      en: 'Manage white-background images, scenes, model shots, references, and video covers in one visual library.',
    },
    stats: [
    ...
    collections: [
      {
        title: { zh: '场景图库', en: 'Scene Library' },
        desc: { zh: '承接 AI 生成场景图、灵感参考和历史沉淀素材。', en: 'Capture AI-generated scenes, inspiration references, and archived visual assets.' },
        meta: { zh: '连接视觉生成链', en: 'Connects to visual generation' },
      },
      {
        title: { zh: '商品素材池', en: 'Product Asset Pool' },
        desc: { zh: '关联 SKU、角度图、主图版本和设计稿源文件。', en: 'Link SKUs, angle shots, hero versions, and source design files.' },
        meta: { zh: '为商品中心服务', en: 'Serves the product center' },
      },
    ],
    actions: [
  ...
  '/brandLibrary': {
    titleKey: 'pages.brandLibrary',
    icon: Bookmark,
    badge: { zh: '品牌资产底座', en: 'Brand Asset Foundation' },
    subtitle: {
      zh: '把品牌规则、语气、视觉标准和营销边界统一沉淀，为 AI 输出约束提供基础。',
      en: 'Consolidate brand rules, tone, visual standards, and marketing boundaries as constraints for AI outputs.',
    },
    stats: [
    ...
    collections: [
      {
        ...
        desc: { zh: '统一管理语气、禁用词、色板、字体和话术习惯。', en: 'Manage tone, forbidden terms, color palettes, fonts, and phrasing habits.' },
        meta: { zh: '服务所有 AI 能力', en: 'Supports all AI capabilities' },
      },
      {
        title: { zh: '品牌输出约束', en: 'Output Constraints' },
        desc: { zh: '未来可作用于模板、Listing、设计器与视频生成。', en: 'Later constrain templates, listings, design, and video generation.' },
        meta: { zh: '统一品牌一致性', en: 'Unified brand consistency' },
      },
    ],
    actions: [
  ...
  '/database/sensitiveThesaurus': {
    titleKey: 'pages.databaseSensitiveThesaurus',
    icon: ShieldAlert,
    badge: { zh: '内容风控规则层', en: 'Content Risk-Control Layer' },
    subtitle: {
      zh: '管理平台风险词、品牌禁用语和触发规则，为内容生成前后质检服务。',
      en: 'Manage risk terms, brand restrictions, and triggers for content QA before and after generation.',
    },
    stats: [
      { value: '216', label: { zh: '风险词', en: 'Risk Terms' } },
      { value: '4', label: { zh: '规则集', en: 'Rule Sets' } },
      { value: '18', label: { zh: '触发样例', en: 'Triggered Cases' } },
    ],
    collections: [
      {
        title: { zh: '规则优先级', en: 'Rule Priorities' },
        desc: { zh: '区分平台级、品牌级、活动级不同的拦截和提醒规则。', en: 'Separate platform, brand, and campaign-level block and warning rules.' },
  ...
  '/database/tagManage': {
    titleKey: 'pages.databaseTagManage',
    icon: Tag,
    badge: { zh: '跨模块标签体系', en: 'Cross-Module Tag System' },
    subtitle: {
      zh: '标签把素材、知识、模板、项目和商品中心串成统一检索语言。',
      en: 'Tags connect assets, knowledge, templates, projects, and product centers into one retrieval language.',
    },
    stats: [
    ...
    collections: [
      {
        title: { zh: '标签层级树', en: 'Tag Hierarchy Tree' },
        desc: { zh: '按渠道、品类、场景、用途和风险等级组织标签结构。', en: 'Organize tags by channel, category, scenario, use case, and risk level.' },
...
export default function AssetCommercePage({ forcedPath }: { forcedPath?: AssetCommercePageKey }) {
  ...
  const visibleUploads = useMemo(() => {
    ...
    return uploads.filter(item => {
      ...
      const matchesSearch =
        !normalized ||
        copyText(locale, item.name).toLowerCase().includes(normalized) ||
        copyText(locale, item.meta).toLowerCase().includes(normalized)
  ...
  const visibleOrders = useMemo(() => {
    ...
    return orders.filter(item => {
      ...
      const matchesSearch =
        !normalized ||
        copyText(locale, item.title).toLowerCase().includes(normalized) ||
        copyText(locale, item.meta).toLowerCase().includes(normalized) ||
  ...
  const getStatusText = (status: UploadStatus | OrderStatus | DeliveryStatus) => {
    switch (status) {
      ...
      case 'pending':
        return locale === 'zh' ? '待支付' : 'Pending'
      case 'active':
        return locale === 'zh' ? '生效中' : 'Active'
      case 'invoice':
        return locale === 'zh' ? '开票中' : 'Invoicing'
      case 'packing':
        return locale === 'zh' ? '打包中' : 'Packing'
      case 'expired':
  ...
  const filterOptions = useMemo(() => {
    if (isOrderPage) {
      return [
        { key: 'all', label: locale === 'zh' ? '全部状态' : 'All States' },
        { key: 'pending', label: getStatusText('pending') },
        { key: 'active', label: getStatusText('active') },
    ...
    if (isDownloadPage) {
      return [
        { key: 'all', label: locale === 'zh' ? '全部状态' : 'All States' },
        { key: 'packing', label: getStatusText('packing') },
    ...
    return [
      { key: 'all', label: locale === 'zh' ? '全部状态' : 'All States' },
      { key: 'queued', label: getStatusText('queued') },
      { key: 'processing', label: getStatusText('processing') },
      { key: 'ready', label: getStatusText('ready') },
    ]
  }, [isDownloadPage, isOrderPage, locale])

  const openDrawer = (payload: { title: string; subtitle: string; notes: string[] }) => {
    setDrawerTitle(payload.title)
    setDrawerSubtitle(payload.subtitle)
    setDrawerNotes(payload.notes)
    setDrawerOpen(true)
  }

  const handleQueueOpen = (item: UploadItem) => {
    openDrawer({
      title: copyText(locale, item.name),
      subtitle: getStatusText(item.status),
      notes: [
        copyText(locale, item.meta),
        locale === 'zh' ? '后续可接真实上传进度、解析日志和团队权限。' : 'Ready for real upload progress, parsing logs, and team permissions.',
        locale === 'zh' ? '当前 mock 已覆盖“加入队列 -> 处理 -> 入库”这条基础链路。' : 'This mock now covers the core flow from queueing to processing to indexing.',
      ],
    })
  }

  const handleOrderOpen = (item: OrderItem) => {
    openDrawer({
      title: copyText(locale, item.title),
  ...
  const handleDeliveryOpen = (item: DeliveryItem) => {
    openDrawer({
      title: copyText(locale, item.title),
      subtitle: `${item.size} · ${getStatusText(item.status)}`,
      notes: [
        copyText(locale, item.meta),
        locale === 'zh' ? '后续可接下载次数、失效时间和重新打包动作。' : 'Ready for download counts, expiry windows, and re-bundling actions.',
        locale === 'zh' ? '适合承接图片、视频、设计稿和批量任务结果。' : 'Suitable for images, videos, design drafts, and batch-task outputs.',
      ],
    })
  }
  ...
  const handlePrimaryAction = () => {
    if (isOrderPage) {
      setOrders(prev => [createMockOrder(prev.length), ...prev])
      return
    }

    if (isDownloadPage) {
      setDeliveries(prev => [createMockDelivery(prev.length), ...prev])
      return
    }
  ...
  const primaryActionLabel = isOrderPage
    ? locale === 'zh'
      ? '模拟创建订单'
      : 'Create Mock Order'
    : isDownloadPage
      ...
      : locale === 'zh'
        ? '模拟上传文件'
        : 'Mock Upload'
  const operationsTitle = isOrderPage
    ? locale === 'zh'
      ? '订单状态流'
      : 'Order State Flow'
    : isDownloadPage
      ? locale === 'zh'
        ? '交付与下载队列'
        : 'Delivery and Download Queue'
      : locale === 'zh'
        ? '上传与入库队列'
        : 'Upload and Index Queue'
  const operationsHint = isOrderPage
    ? locale === 'zh'
      ? '模拟商业订单、支付状态和开票状态的基础骨架。'
      : 'A mock baseline for commercial orders, payment states, and invoice states.'
    : isDownloadPage
      ? locale === 'zh'
        ? '模拟结果打包、下载交付和文件失效流程。'
        : 'A mock baseline for bundling, delivery, and file expiry flows.'
      : locale === 'zh'
        ? '模拟上传、解析、入库和后续权限接入流程。'
        : 'A mock baseline for upload, parsing, indexing, and future permissions.'
  const summaryTitle = isOrderPage
    ? locale === 'zh'
      ? '商业骨架说明'
      : 'Commerce Skeleton Notes'
    : isDownloadPage
      ? locale === 'zh'
        ? '交付骨架说明'
        : 'Delivery Skeleton Notes'
      : locale === 'zh'
        ? '资料骨架说明'
  ...
  return (
    <div className="min-h-screen px-4 py-6 sm:px-6">
          ...
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-xs text-white/55">
                ...
                <span>{copyText(locale, config.badge)}</span>
              </div>
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
                ...
                <div>
                  ...
                  <p className="mt-3 max-w-2xl text-sm leading-6 text-white/55">{copyText(locale, config.subtitle)}</p>
                </div>
              </div>
            </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {config.stats.map(stat => (
            <div key={stat.value + stat.label.zh} className="glass rounded-2xl p-5">
              ...
              <div className="mt-1 text-sm text-white/45">{copyText(locale, stat.label)}</div>
            </div>
          ))}
        </section>
              ...
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                ...
                <span>
                  {isAssetPage
                    ? locale === 'zh'
                      ? '基础检索与资料骨架已接入'
                      : 'Basic retrieval and asset skeleton enabled'
                    : locale === 'zh'
                      ? '基础检索与状态流骨架已接入'
                      : 'Basic retrieval and state-flow skeleton enabled'}
                </span>
                </div>
              </div>
            </div>
              ...
              {visibleCollections.map((item, index) => (
                <article
                  ...
                  className={`tool-card glass rounded-2xl p-5 transition-all ${
                    selectedCollection?.title.zh === item.title.zh ? 'border-brand-500/30 shadow-[0_0_0_1px_rgba(59,130,246,0.25)]' : ''
                  }`}
                >
            ...
            {selectedCollection ? (
              <>
                ...
                <div className="mt-4 rounded-xl border border-white/[0.06] bg-white/[0.03] px-3 py-2 text-xs text-white/40">
                  {copyText(locale, selectedCollection.meta)}
                </div>
                <div className="mt-5 space-y-3">
                  {[
                    locale === 'zh' ? '支持后续接搜索、高级筛选和分页。' : 'Ready for future search, advanced filtering, and pagination.',
                    locale === 'zh' ? '支持接详情抽屉、编辑表单和版本状态。' : 'Ready for detail drawers, edit forms, and version states.',
                    locale === 'zh' ? '支持与聊天、模板、商品中心联动。' : 'Ready to link with chat, templates, and the product center.',
                  ].map(item => (
              ...
              <div className="text-sm text-white/40">
                {locale === 'zh' ? '没有找到匹配项。' : 'No matching item found.'}
              </div>
            )}
          </aside>
        </section>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <div className="flex items-center gap-2 text-sm font-medium text-white/75">
                  ...
                  <span>{operationsTitle}</span>
                </div>
                <p className="mt-2 text-sm text-white/45">{operationsHint}</p>
              </div>
            ...
            <div className="mt-5 space-y-3">
                  ...
                  >
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-white/45">{copyText(locale, item.meta)}</div>
                        <div className="mt-2 text-xs text-white/30">{item.id}</div>
                      </div>
              ...
              {!activeItemsCount && (
                <div className="rounded-2xl border border-dashed border-white/[0.08] bg-white/[0.02] px-4 py-6 text-sm text-white/40">
                  {locale === 'zh' ? '当前筛选条件下没有匹配结果。' : 'No result matches the current filters.'}
                </div>
              )}
            </div>
          </div>
          ...
          <aside className="space-y-4">
            <div className="glass rounded-2xl p-5">
              ...
              <div className="space-y-3">
                {Object.entries(queueSummary).map(([key, count]) => (
                  <div key={key} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3">
            ...
            <div className="glass rounded-2xl p-5">
              <div className="mb-3 flex items-center gap-2 text-sm font-medium text-white/75">
                ...
                <span>{locale === 'zh' ? '已具备的基础能力' : 'Foundation Capabilities Added'}</span>
              </div>
              <div className="space-y-2 text-sm text-white/45">
                {(isAssetPage
                  ? [
                      locale === 'zh' ? '上传队列与入库状态' : 'Upload queue and indexing states',
                      locale === 'zh' ? '搜索与状态筛选' : 'Search and status filters',
                      locale === 'zh' ? '详情抽屉与后续接口说明' : 'Detail drawer and next-step integration hints',
                    ]
                  : isOrderPage
                    ? [
                        locale === 'zh' ? '订单状态与金额展示' : 'Order state and amount display',
                        locale === 'zh' ? '模拟建单与商业流骨架' : 'Mock order creation and commerce-flow skeleton',
                    ...
                    : [
                        ...
                        locale === 'zh' ? '模拟加入交付队列' : 'Mock add-to-delivery queue',
                        locale === 'zh' ? '失效时间与重打包扩展位' : 'Expiry and re-bundling extension points',
                      ]
                ).map(item => (
            ...
            <div className="glass rounded-2xl p-5">
              <div className="mb-3 text-sm font-medium text-white/75">
                {locale === 'zh' ? '跨页回流记录' : 'Cross-page Workflow Feed'}
              </div>
              <div className="space-y-3">
                {workflowEvents.length ? (
                  workflowEvents.map(item => (
                    <div key={item.id} className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3">
                      ...
                      <div className="mt-1 text-xs text-white/45">{copyText(locale, item.detail)}</div>
                    </div>
                  ))
                ) : (
                  <div className="rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] px-4 py-4 text-sm text-white/40">
                    {locale === 'zh' ? '当前还没有跨页回流记录。' : 'No cross-page workflow records yet.'}
                  </div>
                )}
              </div>
            </div>
        ...
        <section className="glass rounded-2xl p-5">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              locale === 'zh' ? '资料权限 / 团队共享' : 'Document permission / team sharing',
              locale === 'zh' ? '订单支付 / 开票 / 退款' : 'Order payment / invoice / refund',
              locale === 'zh' ? '文件打包 / 下载 / 失效时间' : 'Bundle / download / file expiry',
            ].map(item => (
              <div key={item} className="rounded-xl border border-white/[0.06] bg-white/[0.03] px-4 py-3 text-sm text-white/45">
                {item}
              </div>
      ...
      <DetailDrawer open={drawerOpen} title={drawerTitle} subtitle={drawerSubtitle} onClose={() => setDrawerOpen(false)}>
        {drawerNotes.map(item => (
          <div key={item} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] px-4 py-3 text-sm leading-6 text-white/55">
            {item}
          </div>
        ))}
      </DetailDrawer>
