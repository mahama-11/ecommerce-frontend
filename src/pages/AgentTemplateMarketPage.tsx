...
import { addFavoriteTemplate,
  copyTemplateToMyTemplates, getTemplateDetail,
...
const PLATFORM_GROUPS = [ {
  ...
  { key: 'walmart',
    emoji: '🏬', zh: '沃尔玛',
    en: 'Walmart', metrics: { zh: 'Walmart', en: 'Walmart' },
  }, {
    key: 'tiktok-shop', emoji: '🎵',
    zh: 'TikTok / TikTok Shop', en: 'TikTok / TikTok Shop',
...
const TERM_LABELS: Record<string, { zh: string; en: string }> = { amazon: { zh: '亚马逊', en: 'Amazon' },
  walmart: { zh: '沃尔玛', en: 'Walmart' }, 'tiktok-shop': { zh: 'TikTok Shop', en: 'TikTok Shop' },
  independent: { zh: '独立站', en: 'DTC Site' }, official: { zh: '官方', en: 'Official' },
  text: { zh: '文本', en: 'Text' }, image: { zh: '图片', en: 'Image' },
  video: { zh: '视频', en: 'Video' }, workflow: { zh: '工作流', en: 'Workflow' },
  chat: { zh: '对话执行', en: 'Chat' }, image_tool: { zh: '图片生成', en: 'Image Tool' },
  video_tool: { zh: '视频生成', en: 'Video Tool' }, batch_pipeline: { zh: '批量流水线', en: 'Batch Pipeline' },
  hybrid_workflow: { zh: '混合工作流', en: 'Hybrid Workflow' }, model_image: { zh: '模特图', en: 'Model Image' },
  product_image: { zh: '商品图', en: 'Product Image' }, workflow_suite: { zh: '套图工作流', en: 'Workflow Suite' },
  model_swap: { zh: '换模特', en: 'Model Swap' }, mannequin_to_model: { zh: '人台转真人', en: 'Mannequin To Model' },
  background_replace: { zh: '换背景', en: 'Background Replace' }, virtual_tryon: { zh: 'AI 试穿', en: 'Virtual Try-On' },
  accessory_on_model: { zh: '商品上身', en: 'Accessory On Model' }, pose_variation: { zh: '姿势裂变', en: 'Pose Variation' },
  product_scene_compositing: { zh: '场景合成', en: 'Scene Compositing' }, product_swap: { zh: '商品替换', en: 'Product Replace' },
  scene_multiplication: { zh: '场景裂变', en: 'Scene Fission' }, scene_asset_generation: { zh: '场景素材生成', en: 'Scene Asset Generation' },
  hand_hold_product: { zh: '手持商品', en: 'Handheld Product' }, product_retouch: { zh: '商品精修', en: 'Product Retouch' },
  clothing_photo_package: { zh: '服装套图', en: 'Clothing Suite' }, product_photo_package: { zh: '商品套图', en: 'Product Suite' },
  resource_pack: { zh: '资源包', en: 'Resource Pack' }, promo_pack: { zh: '活动包', en: 'Promo Pack' },
  recurring: { zh: '周期型', en: 'Recurring' }, cycle_reset: { zh: '周期重置', en: 'Cycle Reset' },
  form_based: { zh: '表单填写', en: 'Form Based' }, guided_workflow: { zh: '引导式流程', en: 'Guided Workflow' },
...
export default function AgentTemplateMarketPage() {
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 50) {
      if (diff > 0) { setActiveBannerIndex(prev => (prev + 1) % featuredTemplates.length)
  ...
  useEffect(() => {
    ...
    void getTemplateDetail(selectedTemplateId, locale) .then(detail => {
        if (detailRequestVersionRef.current === requestVersion) { setSelectedDetail(detail)
        } })
      .finally(() => {
  ...
  const featuredTemplates = useMemo(() => {
    ...
    return featured.length > 0 ? featured.slice(0, 3) : recommendations.slice(0, 3) }, [recommendations])
  const recommendedTemplates = useMemo(() => {
    return recommendations.filter(r => !featuredTemplates.find(f => f.id === r.id)).slice(0, 4) }, [recommendations, featuredTemplates])
  useEffect(() => {
    if (featuredTemplates.length <= 1) return
    const timer = setInterval(() => { setActiveBannerIndex(prev => (prev + 1) % featuredTemplates.length)
    }, 5000)
  ...
  const toggleFavorite = async (templateId: string) => {
    ...
    setFavoriteIds(prev => wasFavorited ? prev.filter(id => id !== templateId) : [...prev, templateId],
    )
    try {
      if (wasFavorited) {
      ...
      <div className="mx-auto max-w-7xl space-y-6">
          ...
          <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl space-y-4">
              <div className="inline-flex items-center gap-2 rounded-full border border-white/[0.08] bg-white/[0.04] px-3 py-1.5 text-xs text-white/60">
                ...
                <span>{copy(locale, '官方模板中心', 'Official Template Center')}</span> </div>
              <div className="space-y-3">
                <h1 className="text-3xl font-bold sm:text-4xl">
                  <span className="gradient-text">{copy(locale, 'AI Agent 模板市场', 'AI Agent Template Market')}</span> </h1>
                <p className="max-w-2xl text-sm leading-6 text-white/60 sm:text-base">
                  {copy( locale,
                    '围绕跨境电商常见场景，汇聚官方精选模板。你可以按平台、任务类型与使用场景快速找到合适模板，一键收藏、复制并开始使用。', 'Browse curated templates for cross-border ecommerce. Discover the right prompt by platform, task type, and scenario, then favorite, copy, and launch it in one step.',
                  )} </p>
              </div>
              <div className="flex flex-wrap gap-3 text-sm text-white/50">
                <div className="glass rounded-xl px-4 py-3">
                  <div className="text-lg font-semibold text-white">{totalCatalogCount || catalog.length}</div>
                  <div>{copy(locale, '系统模板总数', 'Total Catalog')}</div> </div>
                <div className="glass rounded-xl px-4 py-3">
                  <div className="text-lg font-semibold text-white">{recommendations.length}</div>
                  <div>{copy(locale, '推荐模板', 'Recommended')}</div> </div>
                <div className="glass rounded-xl px-4 py-3">
                  <div className="text-lg font-semibold text-white">{favoriteIds.length}</div>
                  <div>{copy(locale, '已收藏', 'Favorites')}</div> </div>
              </div> </div>
            <div className="glass-strong w-full max-w-md rounded-2xl p-5">
              <div className="mb-4 flex items-center gap-2">
                <Bot className="h-5 w-5 text-brand-400" />
                <div className="font-medium text-white">
                  {copy(locale, '模板亮点', 'Why Templates Help')} </div>
              </div>
              <div className="space-y-3 text-sm text-white/60">
                {[ copy(locale, '按平台、场景、品类和任务快速找到适合的模板。', 'Find the right template by platform, scenario, category, and task.'),
                  copy(locale, '支持收藏常用模板，并复制到自己的模板库继续编辑。', 'Favorite useful templates and copy them into your own library.'), copy(locale, '选择模板后可直接进入对应工具，减少重复填写与试错。', 'Open the right tool directly from a template and skip repeated setup.'),
        ...
        <section className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
          ...
          <div className="space-y-6">
            {featuredTemplates.length > 0 && ( <div
              ...
              >
                {featuredTemplates.map((item, index) => ( <div
                    key={item.id}
                    className={`absolute inset-0 transition-opacity duration-1000 ${ index === activeBannerIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'
                    }`}
                  >
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end p-8 sm:p-12 w-full sm:w-2/3 pb-16">
                      <div className="mb-4 inline-flex w-fit items-center gap-2 rounded-full border border-amber-500/20 bg-amber-500/10 px-3 py-1.5 text-xs text-amber-300">
                        ...
                        <span>{copy(locale, '精选推荐', 'Featured')}</span> </div>
                    ...
                    >
                      <div className="relative aspect-[4/3] overflow-hidden bg-black/20">
                        ...
                        <div className="absolute top-3 right-3 flex items-center gap-1 rounded-full bg-black/60 px-2 py-1 text-[10px] text-amber-400 backdrop-blur-md">
                          ...
                          <span>{item.successRateHint || 98}%</span> </div>
                      </div>
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-[11px] text-white/40">
                            {formatCount(item.useCount ?? 0)} {copy(locale, '次使用', 'uses')} </span>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div className="min-w-[180px]">
                  ...
                  >
                    ...
                    <option value="alphabetical">{copy(locale, '字母排序', 'Alphabetical')}</option> </select>
                </div>
                <div className="flex items-center gap-2 text-xs text-white/40">
                  <Star className="h-3.5 w-3.5 text-amber-400" />
                  <span>{copy(locale, '从模板发现到开始使用，当前页已支持完整浏览与操作流程。', 'This page supports the full flow from discovery to launch.')}</span> </div>
              </div> </div>
            <div className="flex items-center justify-between text-sm text-white/45">
              ...
              <div className="flex flex-wrap items-center justify-end gap-2">
                ...
                {activeSeries && ( <span className="rounded-full border border-brand-500/20 bg-brand-500/10 px-2.5 py-1 text-xs text-brand-300">
                    {copy(locale, '当前系列', 'Active Series')}: {displayFacetLabel(locale, { key: activeSeries, label: activeSeries, count: 0 })} </span>
                )}
                {activeCapability && ( <span className="rounded-full border border-brand-500/20 bg-brand-500/10 px-2.5 py-1 text-xs text-brand-300">
                    {copy(locale, '当前能力', 'Active Capability')}: {displayFacetLabel(locale, { key: activeCapability, label: activeCapability, count: 0 })} </span>
                )} </div>
            </div>
            {facets && ( <div className="grid gap-3 md:grid-cols-4">
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs text-white/35">{copy(locale, '平台分类数', 'Platforms')}</div>
                  <div className="mt-2 text-xl font-semibold text-white">{facets.platforms.length}</div> </div>
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs text-white/35">{copy(locale, '模态分类数', 'Modalities')}</div>
                  <div className="mt-2 text-xl font-semibold text-white">{facets.modalities.length}</div> </div>
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs text-white/35">{copy(locale, '系列分类数', 'Series')}</div>
                  <div className="mt-2 text-xl font-semibold text-white">{facets.series.length}</div> </div>
                <div className="glass rounded-2xl p-4">
                  <div className="text-xs text-white/35">{copy(locale, '能力分类数', 'Capabilities')}</div>
                  <div className="mt-2 text-xl font-semibold text-white">{facets.capabilities.length}</div> </div>
              </div> )}
            {loading ? ( <div className="grid gap-4 md:grid-cols-2">
                {[1, 2, 3, 4].map(i => ( <div key={i} className="tool-card glass rounded-2xl p-5 animate-pulse">
                    ...
                    <div className="mt-5 grid grid-cols-2 gap-2">
                      ...
                      <div className="h-10 rounded-xl bg-white/[0.04]"></div> </div>
                  </div> ))}
              </div> ) : visibleTemplates.length === 0 ? (
              <div className="glass rounded-2xl p-10 text-center">
                <div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/[0.04]">
                  <Search className="h-5 w-5 text-white/35" /> </div>
                <div className="text-lg font-semibold text-white">
                  {copy(locale, '没有找到匹配模板', 'No matching templates')} </div>
                <div className="mt-2 text-sm text-white/45">
                  {copy(locale, '尝试切换平台、清空筛选或修改搜索关键词。', 'Try switching platforms, clearing filters, or adjusting keywords.')} </div>
              </div> ) : (
                  ...
                  >
                    ...
                    <div className="mb-4 flex items-start justify-between gap-3">
                      ...
                      <Button
                        ...
                        className={`inline-flex items-center gap-1 rounded-xl border px-2.5 py-1 text-xs transition-colors ${ isFavorite
                            ? 'border-rose-400/20 bg-rose-400/10 text-rose-300' : 'border-white/[0.08] bg-white/[0.04] text-white/50 hover:text-white'
                        }`}
                      >
                    ...
                    <div className="mb-4 flex flex-wrap gap-2">
                      {[...(card.platformTags ?? []), ...(card.industryTags ?? [])].slice(0, 4).map(tag => ( <span key={tag} className="rounded-full border border-brand-500/20 bg-brand-500/10 px-2.5 py-1 text-[11px] text-brand-300">
                          {displayTerm(locale, tag)} </span>
                      ))} </div>
                    <div className="mb-5 rounded-xl border border-white/[0.06] bg-white/[0.03] p-3 text-xs text-white/45">
                      {templateType(locale, card)} </div>
        ...
        >
          {selectedTemplate ? ( <>
              ...
              {selectedDetail?.examples?.length ? ( <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                    ...
                    {selectedDetail.examples.slice(0, 3).map((example, idx) => ( <div key={(example as Record<string, unknown>).id as string ?? idx} className="overflow-hidden rounded-2xl border border-white/[0.06] bg-[var(--ecom-surface)]">
                        ...
                        <div className="p-3">
                          <div className="text-sm font-medium text-white/75">
                            {((example as Record<string, unknown>).title as string) || copy(locale, '示例预览', 'Example Preview')} </div>
                          <div className="mt-1 text-xs text-white/40">{displayTerm(locale, (example as Record<string, unknown>).exampleType as string | null | undefined)}</div> </div>
                      </div> ))}
                  </div> </div>
              ) : null}
              <div className="flex flex-wrap gap-2">
                {[...(selectedTemplate.platformTags ?? []), ...(selectedTemplate.industryTags ?? []), ...(selectedTemplate.scenarioTags ?? [])].slice(0, 8).map(tag => ( <span key={tag} className="rounded-full border border-brand-500/20 bg-brand-500/10 px-2.5 py-1 text-[11px] text-brand-300">
                    {displayTerm(locale, tag)} </span>
                ))} </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                <div className="mb-2 text-sm font-medium text-white/75">
                  {copy(locale, '适用场景', 'Best-fit Scenario')} </div>
                <div className="text-sm leading-6 text-white/50">
                  {detailLoading ? copy(locale, '正在加载详情...', 'Loading detail...')
                    : selectedDetail?.locale?.scenarioDescription || selectedDetail?.locale?.description || selectedTemplate.summary} </div>
              </div>
              {selectedDetail?.locale?.inputDescription && ( <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-2 text-sm font-medium text-white/75">
                    {copy(locale, '输入说明', 'Input Guidance')} </div>
                  <div className="text-sm leading-6 text-white/50">{selectedDetail.locale?.inputDescription}</div> </div>
              )}
              {selectedDetail?.locale?.outputDescription && ( <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-2 text-sm font-medium text-white/75">
                    {copy(locale, '输出说明', 'Output Guidance')} </div>
                  <div className="text-sm leading-6 text-white/50">{selectedDetail.locale?.outputDescription}</div> </div>
              )}
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3">
                  <div className="text-white/35">{copy(locale, '累计使用', 'Usage')}</div>
                  <div className="mt-1 font-semibold text-white">{formatCount(selectedTemplate.useCount ?? 0)}</div> </div>
                <div className="rounded-xl border border-white/[0.06] bg-white/[0.03] p-3">
                  <div className="text-white/35">{copy(locale, '模板评分', 'Rating')}</div>
                  <div className="mt-1 font-semibold text-white">{selectedTemplate.successRateHint}%</div> </div>
              </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                <div className="mb-2 text-sm font-medium text-white/75">
                  {copy(locale, '模板状态', 'Template Status')} </div>
                <div className="space-y-2 text-sm text-white/50">
                  ...
                  {selectedDetail?.version?.sourceAssetRef && ( <div className="break-all">
                      {copy(locale, `来源规范: ${selectedDetail.version.sourceAssetRef}`, `Spec source: ${selectedDetail.version.sourceAssetRef}`)} </div>
                  )} </div>
              </div>
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                <div className="mb-2 text-sm font-medium text-white/75">
                  {copy(locale, '执行配置', 'Execution Setup')} </div>
                <div className="space-y-2 text-sm text-white/50">
                  <div>{readOutputSummary(locale, selectedDetail)}</div>
                  <div>
                    {copy( locale,
                      `支持异步任务: ${readBoolean(selectedDetail, 'supportsAsyncJob') ? '是' : '否'}`, `Supports async job: ${readBoolean(selectedDetail, 'supportsAsyncJob') ? 'Yes' : 'No'}`,
                    )} </div>
                  <div>
                    {copy( locale,
                      `支持批量执行: ${readBoolean(selectedDetail, 'supportsBatch') ? '是' : '否'}`, `Supports batch execution: ${readBoolean(selectedDetail, 'supportsBatch') ? 'Yes' : 'No'}`,
                    )} </div>
                  <div>
                    {copy( locale,
                      `工具标识: ${readExecutionFlag(selectedDetail, 'toolSlug') || '-'}`, `Tool slug: ${readExecutionFlag(selectedDetail, 'toolSlug') || '-'}`,
                    )} </div>
                </div> </div>
              {inputFields.length > 0 && ( <div className="rounded-2xl border border-white/[0.06] bg-white/[0.03] p-4">
                  <div className="mb-3 text-sm font-medium text-white/75">
                    {copy(locale, '输入字段', 'Input Fields')} </div>
                  <div className="space-y-2">
                    {inputFields.slice(0, 6).map((field, index) => {
                      ...
                      return ( <div key={key} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2 text-sm">
                          <div className="min-w-0">
                            <div className="truncate text-white/75">{key}</div>
                            <div className="text-xs text-white/35">{type}</div> </div>
                          <span className={`rounded-full px-2 py-1 text-[11px] ${required ? 'bg-rose-500/10 text-rose-300' : 'bg-white/[0.06] text-white/45'}`}>
                            {required ? copy(locale, '必填', 'Required') : copy(locale, '可选', 'Optional')} </span>
                        </div> )
                    })} </div>
                </div> )}
              <div className="space-y-2">
