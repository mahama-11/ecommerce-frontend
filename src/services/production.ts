...
import type { StartParsingRequest, StartParsingResponse, EvaluateDecisionTreeRequest, CompileIntentRequest, CompiledIntent, ExecutionConfig, TaskQuota, AssetVariant, CreateInpaintTaskRequest, InpaintTask, RefinementSession, RefinementMessage, SendRefinementMessageRequest, FinalizeAssetsRequest, DualTrackParsing, LlmDecisionTreeResult, ParsingSource, ParsedAttribute, IntentType, VersionNode, ProductionFanoutTask, ProductionFanoutBatch, DecisionOption, DecisionStep, } from '@/types/production'
import { isDevMode, delay, uid, mockDualTrackParsing, mockDecisionTree, MOCK_INTENTS, MOCK_EXECUTION_CONFIG, MOCK_QUOTA, MOCK_VARIANTS, mockInpaintTask, mockRefinementSession, mockRefinementReply, } from '@/mocks/productionDemo'
const VWF = '/api/v1/ecommerce/v2/visual-workflows'
const PRODUCT_VWF = '/api/v1/ecommerce/products'
let devIntentCounter = 4
import type { VisualSession, ProductionTemplateListItem, SourceReferenceDTO, DeconstructionJobDTO, DeconstructionElementDTO, IntentSpecDTO, GenerationVersionDTO, GenerationFanoutResponseDTO, StageViewDTO, BusinessWorkflowDAG, IntegrationVerdict, RollbackSnapshot, ReleaseReadiness } from './production/types'
export type { BusinessWorkflowNode, BusinessWorkflowDAG, IntegrationVerdict, RollbackSnapshot, ReleaseReadiness } from './production/types'
...
function getStored<T>(key: string, fallback: T): T {
  try {
    ...
    return raw ? JSON.parse(raw) as T : fallback } catch {
    return fallback } }
function setStored<T>(key: string, value: T) { localStorage.setItem(key, JSON.stringify(value)) }
function clearStored(key: string) { localStorage.removeItem(key) }
...
async function resolveProductionPromptTemplateId(locale = 'zh-CN'): Promise<string> {
  ...
  if (!selected?.id) { contractNeeded('暂时没有可用的图片模板。请稍后重试，或先选择其他模板。') }
  return selected.id }
...
export type PromptDiffSummary = { added: string[]
  ...
  changed: string[]
  status?: string }
export type PromptPlanSummary = { status: string
  source: string
  promptId?: string
  sceneType?: string
...
function humanizeDiffPath(path: unknown): string {
  ...
  const labels: Record<string, string> = { prompt_id: '出图方案', scene_type: '场景类型', tool_slug: '生产工具', locale: '语言', marketplace: '平台', sku_code: 'SKU', product_id: '商品', status: '状态', execution_contract: '执行合同', prompt_diff: '变化说明', source: '来源', variant: '版本', decision: '取舍决策', qa: 'QA 标记', }
...
export async function requestPromptPlanner(productId: string, opts?: { marketplace?: string; locale?: string; templateId?: string; promptVariables?: Record<string, unknown> }): Promise<{ runtimeJobId?: string; status: string }> {
  ...
  const response = await request<{ runtime_job_id?: string; status: string }>(`${VWF}/${session.id}/prompt-planner-jobs`, { method: 'POST', body: JSON.stringify({ marketplace: opts?.marketplace ?? 'amazon', locale, template_id: templateId, prompt_variables: { ...(opts?.promptVariables ?? {}), template_id: templateId, prompt_diff: true }, idempotency_key: `prompt-plan:${session.id}:${Date.now()}`, }), })
...
export async function ensurePromptPlanReady(productId: string, opts?: { marketplace?: string; locale?: string; templateId?: string; promptVariables?: Record<string, unknown>; timeoutMs?: number }): Promise<PromptPlanSummary> {
  ...
  while (Date.now() - startedAt < timeoutMs) { latest = await getPromptPlanSummary(productId)
    if (latest.status === 'ready' && latest.promptId) return latest
    if (['blocked', 'failed', 'contract_needed'].includes(latest.status)) { contractNeeded('出图方案暂时不可用，请回到生产准备页确认图片解析和选择后再试。') }
    await delay(1500) }
  contractNeeded('出图方案整理超时，请稍后刷新重试。') }
function normalizeStatus(status?: string): 'idle' | 'parsing' | 'succeeded' | 'failed' {
  switch ((status ?? '').toLowerCase()) { case 'running': case 'processing': case 'dispatching': case 'queued': case 'pending': case 'created': return 'parsing'
    case 'completed': case 'succeeded': case 'ready': return 'succeeded'
    case 'failed': case 'cancelled': case 'contract_needed': return 'failed'
    default: return 'idle' } }
...
function userFacingText(input: unknown): string {
  const readableObjectText = (value: unknown, depth = 0): string => {
    ...
    if (typeof value === 'object') {
      ...
      for (const key of ['description', 'summary', 'text', 'label', 'title', 'value', 'style', 'shape', 'material', 'color', 'scene', 'background']) {
        ...
        if (text) return text }
      const parts = Object.entries(obj) .filter(([key]) => !/^(id|asset_id|source_reference_id|source_asset_id|element_type|element_key|metadata|bbox|crop|region)$/i.test(key)) .map(([, item]) => readableObjectText(item, depth + 1)) .filter(Boolean)
  ...
  if (/^[{[]/.test(withoutFence)) {
    try {
      ...
      return '图片解析结果已返回，暂不直接展示原始 JSON' } catch {
      return '图片解析结果已返回，暂不直接展示原始 JSON' } }
  return withoutFence .replace(/Manifest-declared SKU visual asset; geometry analysis pending runtime image bytes/gi, '已识别为当前商品图片；系统会在生成时以实物图为准。') .replace(/Manifest-declared SKU asset; geometry not extractable without image bytes\.?/gi, '已识别为当前商品图片；系统会在生成时以实物图为准。') .replace(/Requested element:\s*product geometry analysis pending visual byte ingestion for asset_[A-Za-z0-9_-]+/gi, '当前图片可以作为商品主体，但外形细节还需要在生成前再次确认。') .replace(/Requested element:\s*material analysis pending visual byte ingestion for asset_[A-Za-z0-9_-]+/gi, '当前图片可以作为材质参考，但具体材质还需要确认。') .replace(/Reference asset available:\s*asset_[A-Za-z0-9_-]+\s*\([^)]*\)\s*for comparative visual analysis/gi, '参考图已就绪，可用于对比风格、场景和构图。') .replace(/Visual description/gi, '视觉描述') .replace(/Provider reference description/gi, '参考图解析描述') .replace(/reference_strategy/gi, '参考策略') .replace(/product_fact/gi, '商品事实') .replace(/All visual facts unverified:.*$/gi, '当前还有部分视觉细节无法自动确认，请以你上传的商品图为准。') .replace(/geometry not extractable without image bytes\.?/gi, '图片细节不足，建议确认商品外形。') .replace(/backend/gi, '系统') .replace(/runtime/gi, '生成服务') .replace(/contract[-_ ]needed/gi, '暂不可用') .replace(/prompt[_ ]plan/gi, '生成方案') .replace(/provider/gi, '生成服务') .replace(/manifest/gi, '图片信息') .replace(/SKU asset/gi, '商品图片') .replace(/image bytes/gi, '图片内容') .replace(/product_geometry/gi, '商品形态') .replace(/brand_constraints/gi, '品牌约束') .replace(/material/gi, '材质') .replace(/style/gi, '风格') .replace(/scene/gi, '场景') }
function elementLabel(element: DeconstructionElementDTO): string {
  ...
  const labels: Record<string, string> = { product_info: '图片中的产品信息', background_info: '图片中的背景信息', provider_reference_description: '参考素材解析结果', reference_style: '参考素材风格', product_geometry: '商品形态', geometry: '商品形态', material: '材质质感', style: '视觉风格', scene: '使用场景', brand_constraints: '品牌约束', reference_composition: '参考图构图', scene_reference: '参考图场景', analysis_limitation: '识别提醒', unverified_visual_claim: '待确认视觉点', lighting: '光线氛围', composition: '画面构图', background: '背景环境', }
...
function elementToAttribute(element: DeconstructionElementDTO): ParsedAttribute {
  ...
  return { key: element.element_key || element.id, label: elementLabel(element), value: Array.isArray(value) || typeof value === 'number' || typeof value === 'string' ? userFacingText(value) : userFacingText(JSON.stringify(value)), confidence: normalizeConfidence(element.confidence), editable: true, source: 'third_party', sourceRole, sourceReferenceId: element.source_reference_id, driftFromOriginal: element.readiness === 'confirmed' ? 0 : undefined, } }
function stageToParsing(stage: StageViewDTO): DualTrackParsing {
  ...
  const status = stage.deconstruction_job ? normalizeStatus(stage.deconstruction_job.status) : attributes.length > 0 ? 'succeeded' : 'idle'
  const job = stage.deconstruction_job
  const failureText = userFacingParsingFailure( job?.error_message || (job?.error_code ? `图片识别失败：${job.error_code}` : undefined) || (status === 'failed' && job?.stage_message ? job.stage_message : undefined) || stage.runtime_capability_error?.message, )
  const result = { track: 'third_party' as const, status, attributes, parsedAt: stage.updated_at, error: failureText, }
  return { status, primaryTrack: 'third_party', thirdPartyResult: result, mergedAttributes: attributes, conflicts: [], } }
function userFacingParsingFailure(message?: string): string | undefined {
  const text = String(message ?? '').trim()
  if (!text) return undefined
  const lower = text.toLowerCase()
  if ( lower.includes('traceback') || lower.includes('name \'traceback\' is not defined') || lower.includes('provider_submit_failed') || lower.includes('comfyui bridge request failed') || lower.includes('gemini') || lower.includes('python') ) {
...
function stageToDecisionTree(stage: StageViewDTO): LlmDecisionTreeResult {
  ...
  const dynamicSteps = visibleElements.map((element, idx) => {
    ...
    const options: DecisionOption[] = [ { id: `${element.id}:keep`, label: `保留${dimension.shortLabel}`, description: `把“${optionLabel}”纳入下一步出图约束`, icon: dimension.icon, confidence: normalizeConfidence(element.confidence), semanticAction: 'keep', dimension: dimension.key }, { id: `${element.id}:replace`, label: `调整${dimension.shortLabel}`, description: `以当前 SKU 为主体，替换或弱化该维度的参考效果`, icon: '⇄', semanticAction: 'replace', dimension: dimension.key }, { id: `${element.id}:crop`, label: `裁剪/局部采用${dimension.shortLabel}`, description: `只采用该属性对应的局部区域或构图线索`, icon: '✂️', semanticAction: 'crop', dimension: dimension.key, cropHint: cropHintFromElement(element) }, { id: `${element.id}:drop`, label: '不采用', description: '该属性不会进入本次出图要求', icon: '–', semanticAction: 'drop', dimension: dimension.key }, ]
    return { id: element.id, stepNumber: idx + 1, title: `${dimension.label} · ${elementLabel(element)}`, description: valueText, options, selectedOptionId: element.decision ? `${element.id}:${element.decision}` : (element.selected ? `${element.id}:keep` : undefined), status: (element.confirmed ? 'completed' : idx === activeIndex ? 'active' : 'pending') as 'pending' | 'active' | 'completed', } })
  const steps = fixedSteps.length > 0 ? fixedSteps : dynamicSteps
  return { status: blockers.length > 0 ? 'failed' : (selections.length > 0 || elements.length > 0 ? 'succeeded' : 'idle'), root: { id: 'root', question: fixedSteps.length > 0 ? '本次出图四项选择' : '本轮视觉策略取舍', confidence: elements.length ? elements.reduce((sum, e) => sum + normalizeConfidence(e.confidence), 0) / elements.length : 0, status: steps.every(step => step.status === 'completed') ? 'completed' : 'active', children: steps.map((step) => ({ id: step.id, question: step.title, answer: step.selectedOptionId, confidence: step.options[0]?.confidence ?? 0, selectedOptionId: step.selectedOptionId, options: step.options, status: step.status, children: [], })), }, steps, recommendedActions: blockers.length > 0 ? blockers.map(b => userFacingText(b.message)) : fixedSteps.length > 0 ? ['按四个固定问题选择“要/不要”', '选择结果会转成自然语言并进入本次出图要求', '底部侧重比例会同步影响 SKU 与参考素材的权重'] : ['逐项确认光影/材质/背景/构图等关键属性', '需要局部保留时选择“调整/裁剪”', '已确认的选择会进入出图要求并驱动下一步生成'], overallConfidence: elements.length ? elements.reduce((sum, e) => sum + normalizeConfidence(e.confidence), 0) / elements.length : 0, provider: 'internal', evaluatedAt: stage.updated_at, } }
function fixedPromptQuestionSteps(elements: DeconstructionElementDTO[], selections: NonNullable<IntentSpecDTO['selections']> = []): DecisionStep[] {
  ...
  const scoreElement = (element: DeconstructionElementDTO, kind: 'product' | 'background') => {
    ...
    if (kind === 'product') {
      ...
      if (backgroundHit) score -= 1 } else {
      if (backgroundHit) score += 2
      if (productHit) score -= 1 }
    return score }
  const pick = (role: 'sku' | 'reference', kind: 'product' | 'background') => {
    const candidates = elements .filter((element) => element.source_role === role && !usedElementIds.has(element.id)) .map((element) => ({ element, score: scoreElement(element, kind) })) .sort((a, b) => b.score - a.score)
  ...
  const configs: Array<{ slot: NonNullable<DecisionOption['promptSlot']>; title: string; keepLabel: string; dropLabel: string; fallbackLabel: string; element?: DeconstructionElementDTO; fallback?: DeconstructionElementDTO }> = [ { slot: 'sku_product', title: '要不要保留 SKU 原图里的产品主体？', keepLabel: '要，保留 SKU 产品主体', dropLabel: '不要使用 SKU 产品主体', fallbackLabel: '以 SKU 原图整体识别结果为准', element: pick('sku', 'product'), fallback: roleFallback('sku') }, { slot: 'sku_background', title: '要不要保留 SKU 原图里的背景？', keepLabel: '要，保留 SKU 背景', dropLabel: '不要，改换 SKU 背景', fallbackLabel: '以 SKU 原图整体识别结果为准', element: pick('sku', 'background'), fallback: roleFallback('sku') }, { slot: 'reference_product', title: '要不要把参考素材里的产品元素带入画面？', keepLabel: '要，参考产品元素进入画面', dropLabel: '不要使用参考产品元素', fallbackLabel: '以参考素材整体识别结果为准', element: pick('reference', 'product'), fallback: roleFallback('reference') }, { slot: 'reference_background', title: '要不要采用参考素材里的背景/场景？', keepLabel: '要，采用参考背景场景', dropLabel: '不要采用参考背景', fallbackLabel: '以参考素材整体识别结果为准', element: pick('reference', 'background'), fallback: roleFallback('reference') }, ]
  ...
  return configs.map((item, idx) => {
    ...
    const options: DecisionOption[] = [ { id: `${stepId}:keep`, label: item.keepLabel, description: valueText, icon: '✓', semanticAction: 'keep', dimension: item.slot.includes('background') ? 'background' : 'product_fact', promptSlot: item.slot, fixedPromptQuestion: true, confidence: normalizeConfidence(element?.confidence) }, { id: `${stepId}:drop`, label: item.dropLabel, description: valueText, icon: '–', semanticAction: 'drop', dimension: item.slot.includes('background') ? 'background' : 'product_fact', promptSlot: item.slot, fixedPromptQuestion: true, confidence: normalizeConfidence(element?.confidence) }, ]
    return { id: stepId, stepNumber: idx + 1, title: item.title, description: valueText, options, selectedOptionId, status: (selectedOptionId ? 'completed' : idx === activeIndex ? 'active' : 'pending') as 'pending' | 'active' | 'completed', } }) }
...
function saveLocalSources(productId: string, sources: ParsingSource[]) { setStored(sourceKey(productId), sources) }
export async function listParsingSources(productId: string): Promise<ParsingSource[]> {
  ...
  try {
    ...
    const backendSources = await Promise.all(sourceRefs.map(async (sourceRef) => {
      ...
      return { id: sourceRef.asset_id || sourceRef.id, type: sourceRole === 'reference' ? 'reference_image' : 'sku_image', url: displayUrl, thumbnailUrl: displayUrl, name: String(sourceRef.metadata?.file_name || sourceRef.source_ref || sourceRef.asset_id || sourceRef.id), uploadedAt: stage.updated_at || new Date().toISOString(), assetId: sourceRef.asset_id, assetRelationId: sourceRef.asset_relation_id, sourceReferenceId: sourceRef.id, sourceRole, mimeType: sourceRef.mime_type || 'image/*', } satisfies ParsingSource }))
    ...
    return merged } catch {
    return localSources } }
export async function uploadParsingSource(productId: string, file: File, sourceType: ParsingSource['type'] = 'sku_image'): Promise<ParsingSource> { validateParsingSourceFile(file)
  if (isDevMode()) {
    await delay(600)
    const source: ParsingSource = { id: `local-src-${uid()}`, type: sourceType, url: URL.createObjectURL(file), thumbnailUrl: URL.createObjectURL(file), name: file.name, mimeType: file.type || 'application/octet-stream', sourceRole: sourceType === 'reference_image' ? 'reference' : 'sku', uploadedAt: new Date().toISOString(), }
  ...
  const asset = await request<{ id: string; mime_type: string; file_name?: string }>(`/api/v1/ecommerce/assets/source`, { method: 'POST', body: JSON.stringify({ product_id: productId, sku_code: skuCode, file_name: file.name, mime_type: file.type || 'application/octet-stream', payload, width: dimensions.width, height: dimensions.height, metadata: { frontend_entrypoint: 'pre_generation_hub', source_role: sourceType === 'reference_image' ? 'reference' : 'sku' }, }), })
  const relation = await request<{ id: string }>(`/api/v1/ecommerce/products/${productId}/assets`, { method: 'POST', body: JSON.stringify({ asset_id: asset.id, relation_type: 'source', asset_role: sourceType === 'reference_image' ? 'scene_shot' : 'hero', is_primary: sourceType !== 'reference_image', }), })
  const localPreviewUrl = URL.createObjectURL(file)
  const source: ParsingSource = { id: asset.id, type: sourceType, url: localPreviewUrl, thumbnailUrl: localPreviewUrl, name: file.name, uploadedAt: new Date().toISOString(), assetId: asset.id, assetRelationId: relation.id, sourceRole: sourceType === 'reference_image' ? 'reference' : 'sku', mimeType: asset.mime_type || file.type || 'application/octet-stream', }
...
export async function removeParsingSource(productId: string, source: ParsingSource): Promise<void> {
  const removeLocal = () => {
    const sources = getLocalSources(productId).filter((item) => item.id !== source.id && item.assetId !== source.assetId && item.sourceReferenceId !== source.sourceReferenceId && item.assetRelationId !== source.assetRelationId, )
    saveLocalSources(productId, sources) }
  if (isDevMode()) { removeLocal()
    return }
  const session = await ensureVisualSession(productId)
  const sourceReferenceId = source.sourceReferenceId
  const calls: Promise<unknown>[] = []
  if (sourceReferenceId) { calls.push(request(`${VWF}/${session.id}/source-references/${sourceReferenceId}`, { method: 'DELETE', silent: true })) }
  if (source.assetRelationId) { calls.push(request(`/api/v1/ecommerce/products/${productId}/assets/${source.assetRelationId}`, { method: 'DELETE', silent: true })) }
  if (calls.length > 0) {
...
async function ensureSourceReference(sessionId: string, productId: string, source: ParsingSource): Promise<SourceReferenceDTO> {
  if (source.sourceReferenceId) {
    return { id: source.sourceReferenceId, source_kind: source.type === 'url' ? 'url' : 'product_asset', source_ref: source.assetId || source.url || source.name || source.id, asset_id: source.assetId, asset_relation_id: source.assetRelationId, mime_type: source.mimeType, status: 'ready', metadata: { source_role: source.sourceRole }, } }
  const sourceRole = source.sourceRole ?? (source.type === 'reference_image' || source.type === 'url' ? 'reference' : 'sku')
  return request<SourceReferenceDTO>(`${VWF}/${sessionId}/source-references`, { method: 'POST', body: JSON.stringify({ source_kind: source.type === 'url' ? 'url' : source.assetId ? 'product_asset' : 'upload', source_ref: source.assetId || source.url || source.name || source.id || productId, asset_id: source.assetId, asset_relation_id: source.assetRelationId, mime_type: source.mimeType || (source.type === 'url' ? undefined : 'image/*'), metadata: { frontend_source_id: source.id, source_role: sourceRole, file_name: source.name, }, }), }) }
export async function startParsing(req: StartParsingRequest): Promise<StartParsingResponse> {
  ...
  if (skuSources.length === 0 || referenceSources.length === 0) { contractNeeded('请至少选择一张商品图和一张参考图，然后再开始图片解析。') }
  ...
  for (const source of selectedSources) {
    ...
    if (idx >= 0) { updatedSources[idx] = { ...updatedSources[idx], sourceReferenceId: sourceRef.id } } }
  ...
  for (const sourceRef of sourceRefs) {
    const job = await request<DeconstructionJobDTO>(`${VWF}/${session.id}/deconstruction-jobs`, { method: 'POST', body: JSON.stringify({ source_reference_id: sourceRef.id, idempotency_key: `deconstruct:${session.id}:${sourceRef.id}:${sourceRefIds.join('+')}:${providerKey}`, requested_elements: ['product_geometry', 'material', 'style', 'scene', 'brand_constraints'], metadata: { frontend_entrypoint: 'production-prep', frontend_source_ids: req.sourceIds, frontend_tracks: req.tracks, source_reference_ids: sourceRefIds, image_understanding_policy: 'single_image_per_runtime_job', provider_code: providerCode, }, }), })
    jobs.push(job) }
...
export async function updateAttentionDecision(productId: string, elementId: string, decision: 'keep' | 'replace' | 'drop' | 'crop', targetAssetId?: string, option?: DecisionOption): Promise<void> {
  ...
  if (elementId.startsWith('fixed:') && option?.promptSlot) {
    ...
    nextSelections.push({ element_id: elementId, element_type: slot.includes('background') ? 'background' : 'product_fact', element_key: slot, decision, label: option.label, value: { description: option.description || option.label }, metadata: { fixed_prompt_question: true, prompt_slot: slot, source_role: sourceRole, source_element_id: element?.id, semantic_action: option.semanticAction ?? decision, }, })
    await request(`${VWF}/${stage.session_id || stage.id}`, { method: 'PATCH', body: JSON.stringify({ intent_spec: { ...(stage.intent_spec ?? {}), schema_version: stage.intent_spec?.schema_version ?? 'v1', product_id: productId, selections: nextSelections, metadata: { ...(stage.intent_spec?.metadata ?? {}), updated_from: 'prep-fixed-four-questions', }, }, }), })
    return }
  await request(`${VWF}/${stage.session_id || stage.id}/deconstruction-elements/${elementId}`, { method: 'PATCH', body: JSON.stringify({ selected: decision !== 'drop', decision, target_asset_id: decision === 'replace' || decision === 'crop' ? targetAssetId : undefined, group_path: option?.dimension ? ['visual_attribute', option.dimension] : undefined, rationale: decision === 'replace' ? '以当前商品图为主体，参考图只作风格参考' : decision === 'keep' ? '保留参考图中的视觉效果' : decision === 'crop' ? '仅采用参考图中的局部区域或裁剪线索' : '不采用这一项参考元素', metadata: { updated_from: 'prep-attention-tree', dimension: option?.dimension, semantic_action: option?.semanticAction ?? decision, crop_hint: option?.cropHint, option_label: option?.label, fixed_prompt_question: option?.fixedPromptQuestion, prompt_slot: option?.promptSlot, }, }), }) }
...
function selectionDecisionLabel(decision?: string): string {
  switch ((decision ?? '').toLowerCase()) { case 'keep': return '纳入出图要求：保留参考效果'
    case 'replace': return '纳入出图要求：主体替换为当前 SKU'
    case 'drop': return '不进入出图要求：已排除'
    default: return '待确认' } }
function selectionDescription(selection: NonNullable<IntentSpecDTO['selections']>[number]): string {
  ...
  const value = selection.value ? userFacingText( selection.value.text ?? selection.value.description ?? selection.value.provider_text ?? selection.value.label ?? selection.value.value ?? '' ) : ''
...
function stageToIntents(stage: StageViewDTO, productId: string): CompiledIntent[] {
  ...
  const fromStage = effectiveSelections.map((selection, idx) => ({ id: selection.element_id || `${stage.id}-intent-${idx}`, type: normalizeIntentType(String(stage.intent_spec?.requirements?.type ?? 'scene_generation')), description: selectionDescription(selection), prompt: JSON.stringify(selection.value ?? selection), priority: 'medium' as const, params: { ...(selection.value ?? {}), decision: selection.decision, element_key: selection.element_key, label: selection.label }, status: 'compiled' as const, createdAt: stage.updated_at || new Date().toISOString(), }))
...
export async function compileIntent(req: CompileIntentRequest): Promise<CompiledIntent> {
  ...
  const nextIntent: CompiledIntent = { id: `manual-${existing.length + 1}`, type: req.type, description: req.description, prompt: req.description, priority: req.priority ?? 'medium', params: req.params ?? {}, status: 'compiled', createdAt: new Date().toISOString(), }
  const intents = [...existing, nextIntent]
  const updated = await request<VisualSession>(`${VWF}/${session.id}`, { method: 'PATCH', body: JSON.stringify({ current_stage: 'prompt', intent_spec: { schema_version: 'v1', scene_type: req.type, tool_slug: 'production-pipeline', product_id: req.productId, selections: intents.map((item, index) => ({ element_id: `manual-${index + 1}`, decision: 'keep', label: item.description, value: item.params })), requirements: { prompt: req.description, priority: req.priority ?? 'medium' }, }, }), })
...
function buildSafeGenerationMetadata(intentIds: string[], config?: ExecutionConfig, source = 'sandbox_start_generation'): Record<string, unknown> {
  return { frontend_intent_ids: intentIds, source, ui_execution_config: config ? { requested_engine: config.provider, generation_provider_code: typeof config.providerConfig?.generation_provider_code === 'string' ? config.providerConfig.generation_provider_code : config.provider, max_concurrency: config.maxConcurrency, retry_on_failure: config.retryOnFailure, max_retries: config.maxRetries, timeout_seconds: config.timeoutSeconds, provider_config: config.providerConfig, } : undefined, } }
export type GenerationExecutionStatus = { versionId: string
  status: string
  stage?: string
  progress: number
...
function generationExecutionStatus(version: GenerationVersionDTO): GenerationExecutionStatus {
  ...
  const blockerMessage = version.blockers?.map(blocker => blocker.message).filter(Boolean).join('；') || String((version.metadata?.runtime_result as Record<string, unknown> | undefined)?.error_message ?? '') || String((version.metadata?.runtime_update as Record<string, unknown> | undefined)?.error_message ?? '')
  const message = successful ? `已生成 ${resultAssetCount} 张结果图，可以进入工坊查看。` : failed ? (blockerMessage || '本次生产没有成功完成，系统没有展示占位图。请检查生成方案或稍后重试。') : progress > 0 ? `正在出图，当前进度约 ${progress}%。请保持本页打开，结果返回后会自动进入工坊。` : '生产任务已提交，正在等待生成服务返回进度。请保持本页打开。'
  return { versionId: version.version_id, status, stage, progress, resultAssetCount, resultAssetUrls: version.result_assets?.map(asset => asset.asset_content_url).filter((url): url is string => Boolean(url)), runtimeJobId: version.runtime_job_id, terminal, successful, message, } }
...
export async function waitForGenerationResult(productId: string, versionId: string, opts?: { timeoutMs?: number }): Promise<GenerationExecutionStatus> {
  ...
  while (Date.now() - startedAt < timeoutMs) { latest = await getGenerationExecutionStatus(productId, versionId)
    if (latest.successful && latest.resultAssetCount > 0) return latest
    if (latest.terminal) { contractNeeded(latest.message || '本次出图已结束，但没有返回可展示结果。') }
    await delay(2500) }
...
export async function executeFanoutIntents( productId: string, intentIds: string[], tasks: ProductionFanoutTask[], config?: ExecutionConfig, opts?: { onProgress?: (batch: ProductionFanoutBatch) => void }, ): Promise<ProductionFanoutBatch> {
  if (isDevMode()) {
    ...
    const devBatch = { batchId, productId, intentIds, tasks: tasks.map((task) => ({ ...task, versionId: `gv-${uid()}`, runtimeJobId: `runtime-${uid()}`, status: 'queued' as const, progress: 5 })), status: 'queued' as const, totalTasks: tasks.length, completedTasks: 0, failedTasks: 0, createdAt: new Date().toISOString(), maxConcurrency: config?.maxConcurrency, retryOnFailure: config?.retryOnFailure, maxRetries: config?.maxRetries, waves: 1, }
  ...
  for (let attempt = 0; pending.length > 0; attempt += 1) {
    ...
    for (let offset = 0; offset < pending.length; offset += maxConcurrency) {
      ...
      const settled = await waitForFanoutWave(productId, submitted.tasks, timeoutMs, (latest) => { opts?.onProgress?.(summarizeFanoutBatch(batchId, productId, intentIds, mergeFanoutProgress(tasks, latest.tasks), { maxConcurrency, retryOnFailure, maxRetries, waves: waveCount })) })
      for (const task of settled.tasks) {
        if (task.status === 'succeeded') { completed.set(task.id, task) } else if (task.status === 'failed') { nextFailures.push({ ...task, retryCount: attempt + 1 }) } else { nextFailures.push({ ...task, status: 'failed', error: task.error || '任务超时，本轮没有返回可展示结果。', retryCount: attempt + 1 }) } } }
    if (!retryOnFailure || attempt >= maxRetries || nextFailures.length === 0) { nextFailures.forEach(task => finalFailures.set(task.id, task))
      break }
...
async function submitFanoutWave(sessionId: string, batchId: string, productId: string, intentIds: string[], tasks: ProductionFanoutTask[], config: ExecutionConfig | undefined, attempt: number, wave: number): Promise<ProductionFanoutBatch> {
  const response = await request<GenerationFanoutResponseDTO>(`${VWF}/${sessionId}/generation-version-fanouts`, { method: 'POST', body: JSON.stringify({ idempotency_key: `${batchId}:attempt-${attempt}:wave-${wave}`, template_slots: tasks.map(task => ({ source_asset_id: task.sourceId, template_id: task.templateId, scene_tag: task.sceneTag, detail_requirement: task.detailRequirement, negative_requirement: task.negativeRequirement, })), source_asset_ids: Array.from(new Set(tasks.map(task => task.sourceId).filter(Boolean))), template_ids: Array.from(new Set(tasks.map(task => task.templateId).filter(Boolean))), requested_variants: 1, max_concurrency: config?.maxConcurrency, retry_on_failure: config?.retryOnFailure, max_retries: config?.maxRetries, timeout_seconds: config?.timeoutSeconds, provider_config: config?.providerConfig, prompt_variables: { ...(typeof config?.providerConfig?.prompt_composer === 'object' && config.providerConfig.prompt_composer !== null ? { prompt_composer: config.providerConfig.prompt_composer } : {}), fanout_slot_prompts: tasks.map(task => ({ slot_index: task.slotIndex, template_id: task.templateId, template_name: task.templateName, scene_tag: task.sceneTag, detail_requirement: task.detailRequirement, negative_requirement: task.negativeRequirement, })), }, metadata: { ...buildSafeGenerationMetadata(intentIds, config, 'sandbox_generation_fanout'), fanout_batch_id: batchId, fanout_attempt: attempt, fanout_wave: wave, }, }), })
  ...
  const nextTasks = response.items.map((item, index) => {
    ...
    return { ...(original ?? { id: item.fanout_task_id, sourceId: item.source_asset_id, templateId: item.template_id, slotIndex: item.slot_index, sceneTag: item.scene_tag, detailRequirement: item.detail_requirement, }), id: original?.id ?? item.fanout_task_id, versionId: version.version_id, runtimeJobId: version.runtime_job_id, status: ['queued', 'processing'].includes(String(version.status).toLowerCase()) ? 'queued' : String(version.status).toLowerCase() === 'completed' ? 'succeeded' : 'failed', progress: Number(version.progress ?? 5), resultAssetCount: version.result_assets?.length ?? 0, resultAssetUrls: version.result_assets?.map(asset => asset.asset_content_url).filter((url): url is string => Boolean(url)), error: version.status === 'contract_needed' ? '生成服务暂时没有开始任务' : undefined, retryCount: original?.retryCount ?? attempt, } as ProductionFanoutTask })
  return summarizeFanoutBatch(response.fanout_id, productId, intentIds, nextTasks, { maxConcurrency: config?.maxConcurrency, retryOnFailure: config?.retryOnFailure, maxRetries: config?.maxRetries, waves: wave, }) }
async function waitForFanoutWave(productId: string, tasks: ProductionFanoutTask[], timeoutMs: number, onProgress?: (batch: ProductionFanoutBatch) => void): Promise<ProductionFanoutBatch> {
  ...
  const startedAt = Date.now()
  while (Date.now() - startedAt < timeoutMs) { current = await getFanoutBatchStatus(productId, current)
    onProgress?.(current)
    const terminal = current.tasks.every(task => task.status === 'succeeded' || task.status === 'failed')
    if (terminal) return current
    await delay(3000) }
  return summarizeFanoutBatch(current.batchId, productId, current.intentIds, current.tasks.map(task => ( task.status === 'succeeded' || task.status === 'failed' ? task : { ...task, status: 'failed', error: '任务超时，未在配置的 timeoutSeconds 内完成。' } ))) }
function summarizeFanoutBatch(batchId: string, productId: string, intentIds: string[], tasks: ProductionFanoutTask[], options?: Partial<ProductionFanoutBatch>): ProductionFanoutBatch {
  ...
  const status: ProductionFanoutBatch['status'] = failedTasks > 0 && completedTasks + failedTasks === tasks.length ? 'failed' : completedTasks === tasks.length ? 'succeeded' : tasks.some(task => task.status === 'executing' || task.status === 'queued') ? 'executing' : 'pending'
...
export async function getFanoutBatchStatus(productId: string, batch: ProductionFanoutBatch): Promise<ProductionFanoutBatch> {
  ...
  const nextTasks = await Promise.all(batch.tasks.map(async (task) => {
    ...
    return { ...task, runtimeJobId: latest.runtimeJobId ?? task.runtimeJobId, status: latest.successful ? 'succeeded' : latest.terminal ? 'failed' : latest.progress > 5 ? 'executing' : 'queued', progress: latest.progress, resultAssetCount: latest.resultAssetCount, resultAssetUrls: latest.resultAssetUrls, error: latest.terminal && !latest.successful ? latest.message : task.error, } as ProductionFanoutTask }))
  return summarizeFanoutBatch(batch.batchId, productId, batch.intentIds, nextTasks) }
export async function createWorkshopGenerationVersion( productId: string, parentVersionId: string, weights: Record<string, unknown>, refinementInstruction: string, source: 'workshop_regenerate' | 'workshop_branch_generation' = 'workshop_branch_generation', ): Promise<{ jobId: string; versionId: string; status: string; runtimeJobId?: string }> {
  if (isDevMode()) {
    await delay(1200)
    return { jobId: `workshop-${uid()}`, versionId: `gv-${uid()}`, status: 'queued' } }
  const session = await ensureVisualSession(productId)
  const promptPlan = await ensurePromptPlanReady(productId, { promptVariables: { source, parent_version_id: parentVersionId, source_version_id: parentVersionId, refinement_instruction: refinementInstruction || 'Workshop regeneration', ui_refinement_weights: weights, prompt_diff: true, }, })
  if (!promptPlan.promptId) { contractNeeded('出图方案还没准备好，请先生成或刷新方案后再试。') }
  const response = await request<GenerationVersionDTO>(`${VWF}/${session.id}/generation-versions`, { method: 'POST', body: JSON.stringify({ prompt_id: promptPlan.promptId, parent_version_id: parentVersionId, source_version_id: parentVersionId, refinement_instruction: refinementInstruction || 'Workshop regeneration', status: 'queued', stage: 'queued', progress: 0, idempotency_key: `${source}:${session.id}:${parentVersionId}:${promptPlan.promptId}:${Date.now()}`, metadata: { source, parent_version_id: parentVersionId, source_version_id: parentVersionId, ui_refinement_weights: weights, prompt_id: promptPlan.promptId, }, }), })
  if (response.status === 'contract_needed') { contractNeeded('本次生成没有成功开始。系统不会展示占位图，请稍后重试。') }
  if (!response.runtime_job_id) { contractNeeded(`本次生成还没有开始，当前状态：${response.status}。`) }
  return { jobId: response.runtime_job_id || response.version_id, versionId: response.version_id, status: response.status, runtimeJobId: response.runtime_job_id, } }
export async function createBranchGenerationVersion( productId: string, parentVersionId: string, weights: Record<string, unknown>, refinementInstruction?: string, ): Promise<{ jobId: string; versionId: string; status: string; runtimeJobId?: string }> {
...
export async function listGenerationVersions(productId: string): Promise<VersionNode[]> {
  ...
  const grouped = Array.from(groups.entries()).map(([id, groupVersions]) => {
    ...
    return { id, versions: sortedGroup, representative, latest } }).sort((a, b) => String(a.latest.created_at ?? '').localeCompare(String(b.latest.created_at ?? '')))
...
export async function saveVariantAsTemplate(productId: string, variantId: string, title?: string): Promise<{ templateId: string; savedTemplates: number }> {
  ...
  if (!versionId || !assetId) { contractNeeded('请先选择一张已生成的图片，再保存为模板。') }
  const result = await request<{ template: { id: string }; saved_templates?: unknown[] }>(`${VWF}/${session.id}/generation-versions/${versionId}/save-as-template`, { method: 'POST', body: JSON.stringify({ asset_id: assetId, title: title || 'Workshop generated visual template', platform: 'ecommerce', tags: ['workshop', 'generated-result'], idempotency_key: `save-template:${session.id}:${versionId}:${assetId}`, }), })
...
export async function finalizeAssets(req: FinalizeAssetsRequest): Promise<{ assetIds: string[]; assetRelationIds: string[] }> {
  ...
  for (const variantId of req.variantIds) {
    ...
    await request(`${VWF}/${session.id}/generation-versions/${versionId}/select`, { method: 'POST', body: JSON.stringify({ selected_result_asset_id: assetId, metadata: { frontend_variant_id: variantId } }), })
    const result = await request<{ asset_relation: { id?: string; asset_id: string } }>(`${VWF}/${session.id}/generation-versions/${versionId}/writeback-selected-asset`, { method: 'POST', body: JSON.stringify({ asset_id: assetId, asset_role: req.assetRoles[variantId] ?? 'hero', idempotency_key: `writeback:${session.id}:${versionId}:${assetId}` }), })
